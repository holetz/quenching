...
description: Turn the /.knowledge/ bundle into a rendered Diátaxis site — planned against a closed publication map, sourced, reviewed and gated by a strict build.
...
<span class="q-badge">Generator: Zensical</span>
<span class="q-badge">Gate: strict build + rendered checks</span>

Your `/.knowledge/` bundle holds the truth; this pipeline turns it into a site a newcomer can
...
```bash
cq knowledge project .knowledge --config zensical.toml --write   # materialize route + snippet
cq knowledge project .knowledge --config zensical.toml --check   # prove hash, route, nav, origin
```
...
!!! abstract "TL;DR for agents"
    ...
    - Never: hand-edit generated routes/snippets; link `/.knowledge/` paths from published
