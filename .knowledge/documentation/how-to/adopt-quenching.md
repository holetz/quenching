...
<span class="q-badge">One OK per front</span>
...
thinning `CLAUDE.md` into a thin pointer over `/.knowledge/`, and backfilling the glossary —
...
??? note "What 'converged' looks like on disk"
    ...
    a root `/.knowledge/index.md` with `okf_version: "0.1"`, a fixed `glossary.md`, and the
...
```bash
cq knowledge validate .knowledge     # 0 error(s) is the gate
