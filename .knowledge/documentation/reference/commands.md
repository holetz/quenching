...
Acts on the `/.knowledge/` OKF bundle.

| Command | Does |
| --- | --- |
| `/quenching:knowledge:align` | Forces `/.knowledge/` into the canonical OKF bundle and pulls in out-of-band content — probe first, ONE confirmed plan, looped to a fixpoint. |
...
| `/quenching:specs:conclude` | Closes a spec out, **merging last**: branch review, emergent `/.knowledge/`, archive (`outcome:`), distillation, `merge:` stamp — then the merge, and nothing after it. |
