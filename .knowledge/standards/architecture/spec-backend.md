...
`/quenching:specs:execute` anchors a task to its commit by the sha recorded in the document. What is kept is
...
**`workItemType` fails the twin test on purpose, and that is why it stays in the document on
EVERY backend — never stripped, unlike the four fields above.** It is a fifth first-level key,
but the opposite shape: `tags`/`assignee`/`start`/`target` are stripped because a faithful native
counterpart exists to hold them; `workItemType` has none, because the value it carries is an
ABSTRACT catalogue key (`.claude/quenching.json`'s `workItemTypes`), and a native tracker has no
field for an abstract key at all — only for a concrete type name. The key and the name are not the
same fact: `incidente` and `Bug` differ by a translation table, exactly the argument that keeps
`date:` in the document where no backend had an honest native counterpart for it either.

**The concrete name IS still projected — an ordinary PROJECTION, not the `spec:` rendering.**
`create_spec` resolves the catalogue entry's native name (`azure`/`github`) once, at creation, and
hands it to the tracker in the same call that was already being made — free, the same argument
that makes `spec:` labels cost nothing extra, but it is not §Rendering derived state: a rendering
is RECALCULATED FROM THE SOURCE ON EVERY WRITE, and this name is resolved exactly once, at `new`,
never again — `az boards work-item update` has no `--type` flag, so nothing in this plugin could
...
`azure-boards`'s `create_spec` has now been measured, by suportar-tipo-workitem-azure-por-tags
(task 4.3), **not to be atomic between creating the work item and patching its board column.** A
type the team's board does not accept (`Bug`, against a board that only maps `User Story`) creates
the item first — it exists, unfiled, in no column — and only then raises `sp-az-no-board` from
`_resolve_board_field`. This is not a defect this spec introduced: a repository always had exactly
one `work_item_type`, so its board coverage was presumably decided once and did not vary create to
create. Resolving the type per spec makes the same pre-existing gap reachable per create instead of
per repository. No fix is proposed here — closing it would mean either a rollback (deleting an item
already visible to the team) or a pre-check against `allowedMappings` before the create call, and
neither was this spec's `## Impact`.
