...
| Doc | Covers |
| --- | --- |
| [align-surface.md](align-surface.md) | The aligned-front column — one align per local front carrying its content stages, the fifth pillar (`git`) declared with no align because it ships no verifier a probe could run, the probe-before-inventory rule that makes a no-op align cost a couple of tool calls, and the rule that no sweep records itself |
| [bundle-root.md](bundle-root.md) | The OKF bundle lives at the fixed `/.knowledge/` root and the design source at `/.design/` — neither root is configurable, because an LLM executor runs command bodies literally and a root it must resolve from config is a root it can resolve wrong |
| [design-front.md](design-front.md) | A fonte DTCG única, suas projeções portáteis e editoriais, a arbitragem explícita com o Impeccable e o limite entre drift web e não-web |
...
| [install-profiles.md](install-profiles.md) | A profile declares which of the four fronts (plus the fifth entry, the `git` pillar) a repository uses, in `.claude/quenching.json` — it turns residency on and off (`disable-model-invocation: true`), never a command or a file; `/align` runs installed local fronts in dependency order, and `git` carries no conductor to skip |
