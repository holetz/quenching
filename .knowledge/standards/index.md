...
| [bundle-root.md](architecture/bundle-root.md) | The OKF bundle of a target repo lives at the fixed `/.knowledge/` root — no configuration file names it, because an LLM executor runs command bodies literally and a root it must resolve from configuration is a root it can resolve wrong |
