...
every command that creates, promotes or ranks a spec pays a reindex call to keep it current.
Whether that cost still buys anything is unexamined: the listing duplicates what
`specs.py list`, `specs.py status` and `/specs:status` already derive from disk on demand, and
it is the only reason the OKF validator is pointed at `specs/plans/` with `--listing-root` at all.

Reassess whether the file should stay, shrink to a static pointer, or go â€” and what each of
those costs across `/specs:create`, `/specs:conclude`, `/specs:align` and the `--listing-root`
conformance path.
