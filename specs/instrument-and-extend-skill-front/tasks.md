...
- [ ] 1.1 Create `assets/bin/skills.py` to the `specs.py` mold — stdlib only, argparse subcommands,
...
- [ ] 1.2 Implement `skills.py lint [path]` and the `sk-*` finding codes — description +
...
- [ ] 1.3 Implement `skills.py doctor` — the bijection (every skill has a wrapper, every wrapper
...
- [ ] 1.4 Implement `skills.py registry reindex` — regenerate the GENERATED zone from
...
- [ ] 1.5 Implement `skills.py budget` — per-skill and total always-on metadata characters
...
- [ ] 1.6 Dogfood on this plugin's own surface: record the baseline `lint`, `doctor`, and `budget`
      output before any skill is edited, so section 3's reduction is measured against a number and
      not an impression
      verify: python assets/bin/skills.py budget --json
- [ ] 1.7 Verify section 1: a throwaway target repo with a non-canonical name, a missing wrapper,
...
- [ ] 2.1 Rewrite `quenching-skill-new` step 7 (OKF tail) and step 8 (self-check) to call
...
- [ ] 2.2 Rewrite `quenching-skill-align` steps 1 and 5 to drive off `skills.py doctor --json` for
...
- [ ] 2.3 Rewrite `quenching-skill-align-and-update` step 2 so the mechanical half of the doctrine
...
- [ ] 2.4 Update `skills/quenching-skill-new/references/taxonomy.md` §registry to cite
...
- [ ] 2.5 Add the `.claude/` row's verifier to `skills/quenching-align-all/references/sweep-doctrine.md`
      §6 — `skills.py lint` + `doctor`, replacing the prose verifier
      files: skills/quenching-align-all/references/sweep-doctrine.md
- [ ] 2.6 Teach `quenching-skill-align` to install `skills.py` into a target's `.claude/hooks/`
      under the same version-comparison rule `quenching-specs-align` uses for `specs.py`
      files: skills/quenching-skill-align/SKILL.md
- [ ] 2.7 Verify section 2: every `skills.py` invocation named in the three skills exists in the
      tool's `--help`, and no skill still describes generating the registry table by hand

## 3. Put the surface on a measured diet

- [ ] 3.1 Bring `quenching-specs-plan-refine` (1,568) and `quenching-specs-plan-apply` (1,559)
      under the 1,536-character cap without removing a trigger phrase
      files: skills/quenching-specs-plan-refine/SKILL.md, skills/quenching-specs-plan-apply/SKILL.md
      verify: python assets/bin/skills.py lint skills --json
- [ ] 3.2 Cut `when_to_use` text that restates the description's `Not for:` boundary across the
      28 shipped skills — frontmatter only, bodies untouched (design.md §Decisions 9)
      verify: python assets/bin/skills.py budget --json
- [ ] 3.3 Flatten the 2-level reference chain: `sweep-doctrine.md` §2's deferral to
...
- [ ] 3.4 Add a table of contents to every `references/*.md` over 100 lines (11 of the 21 files
      today), so a partial read still shows the full scope
- [ ] 3.5 Verify section 3: `budget` reports a total below the 36,651-character baseline recorded
      in 1.6, and `lint` reports zero cap findings
      verify: python assets/bin/skills.py budget --json

## 4. Write the front's standards

- [ ] 4.1 Extend `docs/standards/automation/skills.md` with the invocation/permission decision
      ...
      `authority: current`)
- [ ] 4.2 Write `docs/standards/automation/context-budget.md` — the two caps, what `when_to_use`
      may carry, the per-surface ceiling and its measured baseline, and `skills.py budget` as its
      measurement (`authority: background` until the ceiling is proved — design.md §Open Decisions)
- [ ] 4.3 Update `docs/standards/naming/command-surface.md` — the bijection count at 31 ↔ 31 and
      the wrapper's stated trade-off now that commands and skills are one mechanism, with its
      revisit trigger (`authority: current`)
- [ ] 4.4 Fix `quenching-skill-align`'s own frontmatter under 4.1 — scope its `Bash` grant to the
      commands the workflow actually runs
      files: skills/quenching-skill-align/SKILL.md
      verify: python assets/bin/skills.py lint skills --json
- [ ] 4.5 Update `skills/quenching-skill-new/references/doctrine.md` and `taxonomy.md` to cite the
...
- [ ] 4.6 Update `assets/templates/automation/{skill,command,skills-standard,registry}.md` for
      invocation control, scoped `allowed-tools`, `${CLAUDE_SKILL_DIR}`, and the tool-owned zone
- [ ] 4.7 Verify section 4: `okf-validate.py docs` exits 0, and every standard written here is
...
- [ ] 5.1 Write `skills/quenching-skill-eval/SKILL.md` — authors `evals/evals.json`, runs each case
      ...
      verify: python assets/bin/skills.py lint skills --json
- [ ] 5.2 Write `skills/quenching-skill-eval/references/evaluation.md` — the `evals.json` /
...
- [ ] 5.3 Write the wrapper `commands/skill/eval.md` → `/skill:eval`
      files: commands/skill/eval.md
      pattern: commands/skill/new.md
- [ ] 5.4 Write `docs/standards/automation/skill-evaluation.md` — the evaluation contract: artifact
...
- [ ] 5.5 Verify section 5: run the loop once against an existing skill and commit the resulting
