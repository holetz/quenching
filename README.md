...
| **`quenching-add`** | Adds a **new** piece of knowledge (a standard, catalog table, announcement, …) into the correct home with a complete OKF stamp, updates the folder's `index.md`, appends to `log.md`, enriches the glossary on a new term, and validates. |
| **`quenching-learn`** | Captures one piece of **generic knowledge** the human states (a concept, glossary term, explanation, learning) into the `knowledge/` home with a `type: knowledge` stamp, an updated `index.md`, a `log.md` entry, and a glossary entry for any new term. |
| **`quenching-glossary-backfill`** | Sweeps the **entire** `docs/` bundle for repo-specific terms that already exist in the docs but were never fed into the glossary, fanning out sub-agents per home and backfilling them in one consolidated pass. |
| **`quenching-define`** | Adds or refines **one** entry in the fixed glossary `knowledge/glossary.md` — the repo's A–Z term lookup — placing the entry alphabetically, deriving the link to the concept doc, MERGE never clobber. The on-demand counterpart of the glossary tail step the other knowledge skills run. |
| **`quenching-import-memory`** | Drains the project's Claude Code memory (`~/.claude/projects/<cwd>/memory/`) into the bundle — promoting each memory into its home — then **clears each memory** once its doc lands and passes the conformance check. |
...
/claude-quenching:quenching-add           # add a new standard / table / announcement
/claude-quenching:quenching-learn        # capture a piece of generic knowledge
/claude-quenching:quenching-glossary-backfill   # backfill the glossary from the whole bundle
/claude-quenching:quenching-define         # add / refine one glossary term
/claude-quenching:quenching-import-memory   # drain project memory into the bundle
/claude-quenching:quenching-harness          # refactor CLAUDE.md/AGENTS.md into thin pointers
/claude-quenching:quenching-converge            # loop the sweep skills to OKF convergence
```
