...
| [orchestration-gears.md](automation/orchestration-gears.md) | The gears of the spec orchestrator — the three ways a lifecycle stage runs, the ONE gears plan derived from priority.complexity before any write, and the signals that re-evaluate a gear at the end of a stage and re-authorize the run when it moves up |
