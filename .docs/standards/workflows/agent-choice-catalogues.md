...
for a reader that is not human: `/specs:create` reads it to judge which entry the input best fits,
...
propose the wrong subject, tag or type, confidently, every time `/specs:create` runs against it.
