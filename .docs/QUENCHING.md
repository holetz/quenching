...
(**MERGE** — a filled key and any third-party key survive), regenerates every `index.md`, writes
`okf_version`, installs this manual, then re-runs the validator. It also offers to install or
upgrade the enforcement hook, and — when the repo has a `documentation/` home — an
