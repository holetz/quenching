...
2. The method derives the little that exists (the repo's language for human-only material,
   any initial `CLAUDE.md`) and presents the report with almost all dimensions as gaps.
...
4. Each installed artifact is **adapted** to the repo's prefix and paths — the payload is the
   starting point, not a rigid mold (agent-facing prose stays English; only human-only
   material follows the repo's language).
