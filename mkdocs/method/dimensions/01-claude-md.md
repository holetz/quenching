...
> **Canonical home — the repo's `CLAUDE.md`(s) · Adaptive path, English content.**
> The method derives where they live and bends to the repo's prefixes; it does not
> relocate them. But `CLAUDE.md` is an agent-facing surface, so its prose stays
> **English** (a repo may rename/reorder headings, not translate them). See
