...
name (`docs/standards/`, not `docs/arquitetura/`), with your OK; the agent-facing
surface is English too. An *adaptive* home is derived from the repo — prefixes and
exact paths bend to fit, and product code + human-only material keep the repo's
language. The full doctrine is on the [architecture page](../architecture.md).
