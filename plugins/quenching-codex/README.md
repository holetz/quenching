...
passing the current conversation to a fresh session. Codex auto-routes to a command by its
...
(`/.knowledge/`) → `quenching-components-align` (`.agents/`). The specs lifecycle is provider-owned
...
spec's distillation mints); the two installed fronts run before `.agents/` would otherwise inventory
...
an empty `.agents/` surface skips the automation front with a note. It **conducts, never
