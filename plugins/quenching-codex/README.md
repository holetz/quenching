...
| `quenching-specs-create` | ONE spec in `plans/` — effort proportional to input, never an interrogation. A sentence becomes `## Problem` alone; a Codex plan file becomes every section it actually supports, mapped and never invented. |
...
Codex appends to `PATH` for every enabled plugin, which is what lets a command body write
`cq specs …` bare. `scripts/bin/cq` is the entry point itself, invoked with the plugin root written
...
The order between them, and why `../..` is empty in a shell, are in
...
either bare through `bin/cq` on the PATH or at `../../scripts/cq`, and both are
the same file inside the plugin — so a version bump reaches every consumer the moment Codex
