...
# marketplace instead — which checks 1-3 used to do — serves whatever `~/.agents/plugins/cache/`
...
# checks -> quenching-codex -> plugins -> the repo root, which is where `docs/` and the
# project's own .agents/settings.json live.
...
# Codex may read a cited reference with the dedicated Read tool or with a scoped Bash `cat`.
...
# --------------------------------------------------------------------------- #
# 1. a collapsed command loads and ../.. substitutes in its body
#
