...
# the plugin directory, the way `AGENTS.md` runs it, left `SELF` empty and every pathspec below
...
# nobody retired. That is not hypothetical: this repository's own `.agents/skills/docs/` group
...
# `.agents/skills/`, and it does not live there — so `/specs:develop` names a form that resolves
...
# `../../<path>`, and the same path spelled absolute in a command body — both name a
# file under the plugin. The absolute form appears because ../.. is expanded when
# a body is loaded, and the expansion is what gets copied into prose.
PLUGIN_ROOT_RE    = re.compile(r"\$\{CODEX_PLUGIN_ROOT\}/([A-Za-z0-9_./-]+)")
...
bare_path_re = re.compile(
    r"(?<![A-Za-z0-9_./-])/?(?:\$\{CODEX_PLUGIN_ROOT\}/)?"
