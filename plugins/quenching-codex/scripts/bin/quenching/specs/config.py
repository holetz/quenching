"""Where the workspace is, and what `.agents/quenching.json` declares about it.
...
CONFIG_KEYS = ("backend", "specsBranch", "worktreeSetup", "azureStates",
               ...
               "workItemTypes", "gitConventions", "fanoutMinComplexity", "opsRoot",
               "router")
...
def find_repo_root(specs_root: str) -> str:
    """The target repo's root — where `.agents/` lives.
    ...
    construction: `/.specs/` sits beside `.agents/`, never below it.
...
def load_config(root: str, *, detect_provider_info: bool = True) -> dict:
    ...
    front's alone. `.agents/` is the one directory every front already shares.
    ...
    *wrong* — an unrecognised key, an unrecognised backend, unparseable JSON — comes back as
    ...
    conventions_raw = obj.get("gitConventions")
