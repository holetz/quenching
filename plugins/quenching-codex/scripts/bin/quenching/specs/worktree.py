...
SPECS_WORKTREE_DIR = ".agents/worktrees"
...
def worktree_dir_ignored(cwd: str, rel: str = SPECS_WORKTREE_DIR) -> bool:
    ...
    workspace, and `.agents/worktrees/` relative to `<repo>/.specs/` is a different path that
...
def _inside_worktree_dir(path: str, rel: str = SPECS_WORKTREE_DIR) -> bool:
    ...
    named `.agents/worktrees-archive` is not a worktree."""
...
def specs_lock_path(worktree: str) -> str:
    """`<…>/.agents/worktrees/<name>.lock` — BESIDE the worktree, never inside it.
    ...
    own bookkeeping. Beside, it is already covered by the `.agents/worktrees/` ignore rule that
