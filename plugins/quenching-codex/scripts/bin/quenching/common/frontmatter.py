...
into a target repo's `.agents/hooks/`, "so none may import the others". Nothing installs any
...
def _read_block_scalar(body: list[str], start: int, style: str) -> tuple[str, int]:
    ...
    newline — which is what Codex ultimately reads, so a character count taken here is the
...
def frontmatter_anomalies(text: str) -> list[dict]:
    ...
    and lost prose are byte-identical, and nothing here guarantees Codex's own loader resolves
