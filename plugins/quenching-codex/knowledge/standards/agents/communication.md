...
(`/.knowledge/standards/AGENTS.md`) loads when that folder is touched, not at session start, so it would
...
| How commands, hooks and agent definitions are classified, authored, budgeted and swept | the repo's `/.knowledge/standards/automation/` subject | cites it |
