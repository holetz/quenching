...
1. **`cq …`, bare.** Codex appends `<pluginRoot>/bin` to `PATH` for every enabled plugin, and
...
2. **`python3 "<pluginRoot>/scripts/bin/cq" …`**, with the root written out in full. Use it wherever
...
`.agents/hooks/`, never install one there, never merge anything into a target's
`.agents/settings.json` to make a tool resolve. The two routes are one file reached two ways, never
two installations.
...
The variable is substituted into a **command body's text** when Codex loads it, so a body
citing `../../…` arrives already expanded and its absolute path can be copied
...
```bash
python3 "<pluginRoot>/scripts/bin/cq" specs status --spec my-spec --json
```

The same rule binds anything executable the plugin ships: it locates itself from its own
`__file__`, never from `../..` — which is what `bin/cq` does to find
`scripts/bin/cq`.
...
```bash
CQ="python3 <pluginRoot>/scripts/bin/cq"
