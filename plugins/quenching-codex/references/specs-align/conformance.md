...
<!-- rules -->
`.agents/quenching.json`, at the **repo root** — the `backend` key naming which store holds the
...
| Code | Fires when | Fix |
| --- | --- | --- |
| `sp-no-workspace` **(tool)** | No `specs/`, and no legacy `openspec/` | Offer to scaffold by copying `../../specs/` (both phase folders + template + schema). Declining ends the run. **`files` backend only** — §Where the front is configured. |
...
| `sp-shadow-skill` | `.agents/skills/openspec-<x>/SKILL.md` carries `metadata.generatedBy` **and** the plugin ships a command covering it | Propose removal in the plan; the batch OK covers it. Legacy migration only. |
| `sp-shadow-command` | `.agents/skills/opsx/*.md` is a CLI-generated wrapper for a plugin-shipped command | Propose removal alongside its skill. |
