...
### 5. Stamp, with an ID
```bash
python3 "$(find "${CODEX_HOME:-$HOME/.codex}" "$HOME/.codex" -type f -path '*/quenching-codex*/scripts/cq' -print -quit 2>/dev/null)" specs record "<id>" pr --set number=<provider-pr-id> --set url=<webUrl> --set date=<today>
```
