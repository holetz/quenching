...
- **Order is a dependency, not a preference.** `/.knowledge/` → `/.design/` → `.agents/`.
...
- **Front presence decides the pass; only `/.knowledge/` is installed unasked.** An absent
  ...
  artifacts already signal it; otherwise it is reported as available. An empty `.agents/`
...
## Workflow (probe → ONE OK → three local fronts → re-probe → loop)

### 1. Probe the three local fronts (read-only, cheap)
...
and 3 write into or depend on the bundle.
...
- Never scaffold a `.agents/` taxonomy for an empty surface; never silently install a visual world
