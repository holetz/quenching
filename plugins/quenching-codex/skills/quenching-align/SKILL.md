...
| 3 | `.agents/` — the automation surface | `quenching-components-align` | command paths on the taxonomy axis, collapsed pairs, the rule + registry, the GENERATED zone — then the read-only doctrine audit |
...
([sweep-doctrine](../../references/align/sweep-doctrine.md) §1. Probe before the inventory) — which is what made it safe for each align to carry its own content stages instead
...
- [sweep-doctrine.md](../../references/align/sweep-doctrine.md) — probe
...
- **Order is a dependency, not a preference.** `/.knowledge/` → `/.specs/` → `.agents/`:
  ...
  - **specs before skills** — when migrating a legacy `openspec/` workspace, `quenching-specs-align`
    removes the CLI-generated `.agents/skills/openspec-*` + `.agents/skills/opsx/` shadow
...
- **One OK for the whole repo; authorization nests one level.** The gate fires **once**, before
  ...
  not ask again
  ([convergence.md](../../references/align/convergence.md) §The cycle-authorization contract).
...
- **Front presence decides the pass; only `/.knowledge/` is installed unasked.** An absent `/.knowledge/` bundle
  ...
  never a side effect. An empty `.agents/` surface (no commands, no skills) skips front 3 with a
...
### 5. Front 3 — `quenching-components-align` (the `.agents/` surface)
...
- Never scaffold `/.specs/` without its own explicit line in the plan, and never scaffold a
  `.agents/` taxonomy for an empty surface.
