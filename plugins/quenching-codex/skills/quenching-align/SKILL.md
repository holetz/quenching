...
| `components` — `.agents/` | `quenching-components-align` | `design` |
...
`../../references/front-align/mold.md`. The conductor sequences and reports;
...
- **`ops`** — read `.agents/quenching.json` for `ops.opsRoot`; when absent, inspect only
  `scripts/`, `tools/`, `bin/` and `script/` for an executable entry point.
- **`proof`** — read `.agents/quenching.json` for `proof.proofRoot`; when absent, inspect only
...
- Never scaffold an empty `.agents/` surface or silently install a visual world.
