...
description: "Add or refine ONE entry in the fixed glossary (glossary.md). Triggers on \"add a term to the glossary\", \"define this term\", \"add this acronym/jargon to the glossary\". Not for: a full concept doc → quenching-knowledge-add; sweeping the whole bundle for missing terms → quenching-knowledge-glossary-backfill."
---
...
[`glossary.md`](../../knowledge/glossary.md) — the repo's single A–Z lookup
...
([knowledge-add/homes.md](../../references/knowledge-add/homes.md)); the `type`
...
[knowledge-align/conformance.md](../../references/knowledge-align/conformance.md)). For a whole-bundle bulk
...
- **The glossary is an index, not a home for depth.** An entry is a **one-sentence** meaning plus
  ...
  sentence, the depth belongs in a `concepts/` concept doc (`quenching-knowledge-learn`) and the entry
...
- **Link derived, never invented.** The link targets a doc that actually exists (`/.knowledge/<path>.md`,
...
- **One fixed file.** Everything lands in `glossary.md`. Do not create a per-term file,
  do not touch `concepts/index.md` (the glossary is already listed), and never add frontmatter to
...
`quenching-knowledge-align` to install the skeleton (it ships the fixed glossary), then resume. Read the
...
note that a `quenching-knowledge-learn` capture would give the term a home.
