...
mirror: the command's path IS its identity. The axis, naming, and registry format live in
...
- **No bundle, no tail — but the mint proceeds.** `/docs/index.md` without `okf_version`
  (or absent) means: write the command file only, skip registry/glossary, report that the bundle
  tail did not apply, and suggest `quenching-knowledge-align` **once**.
...
- **The registry zone is regenerated, never composed.** `cq components registry reindex` owns the
  zone's format; the zone is never written between the markers by hand.
...
under `commands/`: shared procedure lives outside it and is cited by absolute path. **Done when:**
the path to be written is fixed, collision-free or resolved as an edit.
...
route a spoken request here. For an edit, apply the doctrine to the diff — cure the named failure mode
...
non-default lever with its reason, or "default"), every file (the command, rule if planned,
  registry create-or-regenerate), and the tail steps. Wait for the single confirmation.
...
python3 "$(find "${CODEX_HOME:-$HOME/.codex}" "$HOME/.codex" -type f -path '*/quenching-codex*/scripts/cq' -print -quit 2>/dev/null)" components registry reindex --json      # `changed: false` — nothing wrote inside the markers after the description pass
```
...
changed. In this plugin that is
