...
description: "Refactor AGENTS.md/AGENTS.md into thin pointers over the /.knowledge/ bundle. Triggers on \"refactor AGENTS.md\", \"slim down AGENTS.md\", \"move AGENTS.md content into docs\", \"align AGENTS.md/AGENTS.md with /.knowledge/\". Not for: writing a standalone doc into the bundle → quenching-knowledge-add."
---

<!-- GENERATED FROM plugins/quenching/commands/components/harness/align.md -->


# quenching-components-harness-align — make AGENTS.md a thin, honest pointer into the bundle

**Input**: `$ARGUMENTS` (an optional specific harness file; omit to sweep every AGENTS.md/AGENTS.md).
...
[knowledge-align/migration.md](../../references/knowledge-align/migration.md)). Harness
molds live at `../../templates/harness/`.
...
- **Move, never copy.** After the run each fact lives in exactly **ONE** place — its concept doc.
  A rule restated in AGENTS.md is the same drift as a lying index; the pointer **cites** the doc,
...
`Glob` `**/AGENTS.md`, `**/AGENTS.md`, and `CLAUDE.local.md`; add a `grep --no-ignore` sweep to
...
Each survivor is a **proposed** subfolder `AGENTS.md`, carried into the plan (step 5) as its own
gated item — never auto-created. See [components-harness-align/harness-routing.md](../../references/components-harness-align/harness-routing.md) §6.
...
[knowledge-add/homes.md](../../references/knowledge-add/homes.md); a **DEDUPE**
...
([knowledge-align/migration.md](../../references/knowledge-align/migration.md) §3 §4):
...
shape** (which mold, which KEEP residue). Include evidence-gated subfolder-`AGENTS.md` creations —
