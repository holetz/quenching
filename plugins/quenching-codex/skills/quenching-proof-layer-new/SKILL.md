...
1. add the named layer to `.agents/quenching.json`'s `proof.layers` map with reach, budget and
...
- Reach and budget live in `.agents/quenching.json`'s `proof.layers` declaration.
