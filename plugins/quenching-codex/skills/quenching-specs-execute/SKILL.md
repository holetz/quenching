...
per [align/tool-resolution.md](../../references/align/tool-resolution.md)
...
```bash
python3 "$(find "${CODEX_HOME:-$HOME/.codex}" "$HOME/.codex" -type f -path '*/quenching-codex*/scripts/cq' -print -quit 2>/dev/null)" specs record "<id>" branch --set base=<resolved base> --set work=<current branch>
```
...
same call as the reads above, per [execution.md](../../references/specs-execute/execution.md)
...
```bash
python3 "$(find "${CODEX_HOME:-$HOME/.codex}" "$HOME/.codex" -type f -path '*/quenching-codex*/scripts/cq' -print -quit 2>/dev/null)" specs section "<id>" --moment build --scope current --json
```
...
```bash
python3 "$(find "${CODEX_HOME:-$HOME/.codex}" "$HOME/.codex" -type f -path '*/quenching-codex*/scripts/cq' -print -quit 2>/dev/null)" specs next --spec "<id>" --json
```
...
c. **Write only the `/.knowledge/` this task names.** When this task writes `/.knowledge/`, load the rule that
   ...
   self-check against [knowledge-align/conformance.md](../../references/knowledge-align/conformance.md)
...
e. **Then run verify, tick and commit as ONE chained call.** Decide the subject first — it follows
   ...
   ```bash
   <the task's verify:> \
     && python3 "$(find "${CODEX_HOME:-$HOME/.codex}" "$HOME/.codex" -type f -path '*/quenching-codex*/scripts/cq' -print -quit 2>/dev/null)" specs task --check <id> --spec "<id>" --subject "plan/<id>-<handle>: <id> <title>" \
   ...
   **The `&&` is the ordering**, not a shortcut around it. Every guarantee the four separate acts
   ...
   when the spec's declared policy says this task is a gate ([execution.md](../../references/specs-execute/execution.md)
...
h. **On a section boundary with no `[!]`, squash and repair** — per §The section squash, loaded once alongside 5d:
   ...
   python3 "$(find "${CODEX_HOME:-$HOME/.codex}" "$HOME/.codex" -type f -path '*/quenching-codex*/scripts/cq' -print -quit 2>/dev/null)" specs task --check <id> --spec "<id>" --subject "plan/<id>-<handle>: <N> <section title>"   # per task
   ```
...
```bash
python3 "$(find "${CODEX_HOME:-$HOME/.codex}" "$HOME/.codex" -type f -path '*/quenching-codex*/scripts/cq' -print -quit 2>/dev/null)" specs section "<id>" Handoff --write --scope global   # the evergreen block
python3 "$(find "${CODEX_HOME:-$HOME/.codex}" "$HOME/.codex" -type f -path '*/quenching-codex*/scripts/cq' -print -quit 2>/dev/null)" specs section "<id>" Handoff --write --scope current  # the block of the open section
```
...
[execution.md](../../references/specs-execute/execution.md) §The commit, and
