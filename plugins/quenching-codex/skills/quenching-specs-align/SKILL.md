...
<!-- GENERATED FROM plugins/quenching/commands/specs/align.md -->
...
`quenching-components-align` its `.agents/` command surface, this one converges its **spec-driven workspace** —
...
to converge. A repo that declares `github` or `azure-boards` in `.agents/quenching.json` may hold no
...
- **A diverged shadow copy is kept.** A legacy `openspec init` generated local
  `.agents/skills/openspec-*` copies of commands this plugin ships; identical ones are removal
  candidates, but one a repo has customized (`sp-shadow-diverged`) is kept and reported — the
  general never-delete-on-a-guess rule, applied to the one `.agents/` surface this sweep may touch,
...
- **Authoring is reported, never supplied.** A present-but-empty heading (`sp-empty-section`), a
  ...
  [conformance](../../references/specs-align/conformance.md)
...
`.agents/skills/openspec-*/SKILL.md` and `.agents/skills/opsx/*.md` (shadow copies, relevant only
...
[align/sweep-doctrine.md](../../references/align/sweep-doctrine.md)'s
...
One plan, in sections: scaffold (copy `../../specs/` into `/.specs/` when
...
- Never delete a **diverged** shadow copy, and never touch any `.agents/` skill or command outside
