---
description: Build a ready spec — verify, review, and commit each task
argument-hint: [slug]
allowed-tools: Bash, Read, Glob, Grep, Write, Edit, AskUserQuestion, Task, Skill
---

# /specs:apply — implement a plan's tasks
...
delegating an executor — live in [specs-apply/execution.md](${CLAUDE_PLUGIN_ROOT}/assets/references/specs-apply/execution.md), which this
...
**Why `Bash` is unrestricted here.** This is the one `/specs:*` skill that runs the
...
2. **Require a clean tree, then offer to isolate (recommended, never imposed)**
   ...
   **Precondition first.** Run `git status --porcelain`. If it is non-empty, **refuse to start**
   ...
   ([execution.md](${CLAUDE_PLUGIN_ROOT}/assets/references/specs-apply/execution.md) §The precondition.)
...
7. **Implement tasks (loop until done or blocked)**
   ...
   b. **Write the code** — minimal and focused, scoped to the task's declared files. A task that
      ...
      and the pause decision ([execution.md](${CLAUDE_PLUGIN_ROOT}/assets/references/specs-apply/execution.md) §Delegating an executor —
   ...
   d. **Verify, per the plan's policy** ([execution.md](${CLAUDE_PLUGIN_ROOT}/assets/references/specs-apply/execution.md) §The validation
   ...
   **Pause if:**
   ...
   - a task conflicts with a `docs/standards/` contract → surface it and let the human pick
     (revise the standard via `/docs:add`, or revise the plan via
     `/specs:develop`);
...
8. **Offer the end-of-plan review**
   ...
   ([execution.md](${CLAUDE_PLUGIN_ROOT}/assets/references/specs-apply/execution.md) §The end-of-plan review): only the whole diff shows
...
9. **On completion or pause, show status — and offer to chain**
   ...
   **At 100%**, offer to chain straight into `/specs:archive` (the `Skill` tool) so
...
- A task written `[!]` is **blocked, not failed-forever** — report it with its reason and move on.
  changing something.
- Delegate an executor only under [execution.md](${CLAUDE_PLUGIN_ROOT}/assets/references/specs-apply/execution.md) §Delegating an executor
...
- **Route other durable learning to its OKF home**: an insight worth keeping beyond this plan (a
  gotcha, a domain understanding, a term) goes through `/docs:learn` /
  `/docs:add` / `/docs:define` — offer the capture, don't auto-write it, and
