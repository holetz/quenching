...
**Input**: `$ARGUMENTS` — a list of spec IDs, or nothing. Omitted → propose the queue from the
...
IDs given → resolve each against that payload, in the order the human typed them; one that does
...
<id>` beside it; one whose `priority` record carries no `complexity` has nothing for the
...
- **the whole list**, in queue order — one row per spec: id · derived stage · `complexity` ·
...
**No flag reaches this plan.** The one argument is a list of IDs: nothing changes mode, effort,
...
Cut **one** ref for the whole queue, named `plan/<the first id>-<handle>` — the name is a suggestion
...
cq components read ${CLAUDE_PLUGIN_ROOT}/assets/references/specs-fanout/fanout.md \
  ...
  --sections "§The branch carries the IDs"
cq components read ${CLAUDE_PLUGIN_ROOT}/assets/references/specs-execute/execution.md \
...
a. **Dispatch ONE `Task` sub-agent, pinned to the session model — never `haiku`.** Its whole
   instruction is to invoke `quenching:specs:execute <id>` through the Skill tool under step 2's
...
c. **Act on the classification** (§Classifying a block):
   ...
   | **local** | leave the executor's `[!]` where it is, take the ID off `quenching-specs:`, move on |
   ...
   An ID comes off the line by the read-merge-write in
...
`quenching-specs:` names exactly what the branch carries.
...
4 specs on plan/<first id>` — because a queue is N specs, not one. Three body blocks:

1. **Built** — fixed. One row per spec: id · tasks checked · the commits it made · the gate's
   verdict after it.
...
```
Queue: 4 specs → plan/<first id> → PR against the primary branch

[2/4] <id> — executing · 6 tasks
✓ execute: 6/6 checked, 2 commits
✓ gate: pnpm test — passed
✓ branch mark: quenching-specs: <id1>,<id2>
```

## Hard rules — no exceptions, and no "just this once"

- **Never leave a blocked spec's ID on `quenching-specs:`.** That line is the whole set
  `conclude` builds the pull request from, so an ID left on it makes the PR claim work it does not
...
- Keep the run flagless: the one argument is a list of IDs, and no invocation flag changes mode,
...
- Write no file of your own. The one write this body makes is the `quenching-specs:` line, and only
  to remove a blocked spec's ID from it.
