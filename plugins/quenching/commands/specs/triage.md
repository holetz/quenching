---
description: Sweep the whole front — backlog stages plus every spec's discoveries
argument-hint: [optional-slug]
allowed-tools: Read, Grep, Glob, Write, Edit, Bash(python3:*), Bash(py:*)
---

# /specs:triage — prioritize the task inbox

**Input**: `$ARGUMENTS` (optionally one slug to limit the sweep; omit to sweep everything).

Sweeps the **entire** task inbox at
[`specs/backlog/`](../../assets/specs/backlog/index.md), and proposes priorities and
themes for what is parked there — the sweep counterpart of the per-item `/specs:capture`
...
Unlike `/docs:glossary-backfill`, this sweep fans **no** sub-agents out: a backlog is small
...
the shared log procedure lives with `/docs:add`
([docs-add/homes.md](${CLAUDE_PLUGIN_ROOT}/assets/references/docs-add/homes.md)). Triage is
**Stage 3 of the `specs/` front's pipeline** — it is not a `docs/`-front stage, and it is not
a per-item tool. **Exception — cycle-authorized runs:** invoked by `/specs:align-and-update`
(or `/align-and-update`) under the cycle-authorization contract
...
stop and offer `/specs:capture` (which installs the seed) or note there is nothing
