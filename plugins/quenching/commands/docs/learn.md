...
# /docs:learn — capture generic knowledge, OKF-conformant
...
- **`knowledge/` is generic understanding, not a contract.** It holds what the team
  ...
  **stop and route via `/docs:add`** rather than mis-filing it under `knowledge/`.
...
- **Feed the glossary.** `knowledge/` ships one fixed file — `glossary.md`, the repo's A–Z term
  ...
  `/docs:glossary-backfill` is the whole-bundle counterpart that backfills terms already sitting
...
to `/docs:add`. Otherwise proceed under `knowledge/`.
...
every capture runs; on-demand counterpart `/docs:define`, bulk counterpart
...
- Never file a **contract / decision / procedure / external-asset fact** under `knowledge/` —
  route it to its home via `/docs:add`.
