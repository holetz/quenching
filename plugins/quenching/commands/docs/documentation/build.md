...
# /docs:documentation:build — create/update the `documentation/` site
...
`/docs:align` step 7 offers the **first** install of this layer as part of scaffolding the
...
do). **No bundle → stop** and offer `/docs:align` first; there is nothing to render. Bundle
but **no `documentation/` home** → stop and offer `/docs:align` (the skeleton ships the home,
...
- Never scaffold the bundle, the `documentation/` home, or a missing `index.md` here → `/docs:align`.
