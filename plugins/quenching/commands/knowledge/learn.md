---
description: Capture ONE piece of generic knowledge into the bundle's knowledge/ home. Triggers on "add this knowledge", "record what we learned", "put this in the knowledge base". Not for: a full contract/decision/procedure/external-asset doc → /quenching:knowledge:add; ONE glossary term → /quenching:knowledge:define.
...
- **`knowledge/` is generic understanding, not a contract.** It holds what the team
  ...
  **stop and route via `/quenching:knowledge:add`** rather than mis-filing it under `knowledge/`.
...
to `/quenching:knowledge:add`. Otherwise proceed under `knowledge/`.
