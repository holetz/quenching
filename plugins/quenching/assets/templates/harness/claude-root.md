...
**glossary first** → [/.knowledge/glossary.md](/.knowledge/glossary.md): the A–Z lookup,
one entry per term with a link to its full doc when one exists. `grep -i '<term>' /.knowledge/glossary.md`.

- [/.knowledge/standards/](/.knowledge/standards/index.md) — how WE build (current, proven contracts).
- [/.knowledge/concepts/](/.knowledge/concepts/index.md) — generic understanding we hold (concepts, learnings);
  its [glossary.md](/.knowledge/glossary.md) is the term lookup.
- [/.knowledge/external/](/.knowledge/external/index.md) — facts about what we consume (tools, libs, regulations).
- [/.knowledge/<other-home>/](/.knowledge/<other-home>/index.md) — <boundary one-liner>.
- [specs/backlog/](specs/backlog/index.md) — task inbox (park now, triage later); a
  quenching-managed sibling of the plan folders in `specs/`, **outside** the `/.knowledge/` bundle. Drop
  this bullet if the repo has no `specs/`.

<!-- HOW TO FIND A TERM: the glossary is the repo's A–Z vocabulary index at
     /.knowledge/glossary.md — a flat, alphabetically sorted bullet list (the same syntax
     ...
     defined — add it with the `quenching-docs-define` skill (one term), backfill many at once
...
Anything that is durable knowledge belongs in a /.knowledge/ home above, cited here, never inlined.>
...
<!-- Root harness pointer, auto-loaded by Claude Code on every turn. Keep it a thin pointer:
     ...
     Installed/maintained by the quenching plugin; this file is a harness pointer, not an
     OKF concept. -->
...
<!-- MOLD (quenching · root harness pointer — do NOT copy this note into the produced file):
     ...
     • NO frontmatter, NO `type` — a harness file is EXEMPT from OKF (okf-spec §strict-7); the
       validator skips it. Pointer honesty is verified by the `quenching-docs-harness` skill, not the
     ...
     • KEEP only what the agent needs on EVERY task (commands, env, etiquette). MOVE durable
       knowledge into its /.knowledge/ home (via the quenching-docs-add procedure) and leave a one-line
     ...
     • KEEP the "Resolving a term" pointer + the HOW-TO-FIND-A-TERM comment when the repo has a
       `glossary.md` — term resolution via the glossary is a habit worth one line every
