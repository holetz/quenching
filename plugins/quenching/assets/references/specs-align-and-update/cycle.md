...
The reference `/specs:align-and-update` reads. It fixes the two things specific to the
...
| --- | --- | --- | --- |
| 1 | `/specs:align` | **structure** | Nothing can be listed, validated, or archived in a workspace that is malformed. Align scaffolds, applies `specs.py doctor`'s declared repairs, canonicalizes plan and archive names, seeds and stamps `backlog/`, regenerates its zone, migrates a legacy `openspec/` workspace, and clears any shadow copies. Everything downstream assumes a `doctor`-clean workspace. |
| 2 | `/specs:archive` | **retire completed work** | The front's real content stage, and the plan↔OKF bridge: archiving a complete plan distils the durable knowledge it produced into `docs/`. Runs before triage so the backlog is ranked against what is actually still open. **Each archive is its own confirmation** ([convergence.md](${CLAUDE_PLUGIN_ROOT}/assets/references/align-and-update-all/convergence.md) §What it NEVER covers) — it is irreversible in the sense that matters: the spec leaves `ready/`, and its `outcome:` is a claim only the human can make. |
| 3 | `/specs:triage` | **sweep stages + discoveries** | Sweeps the backlog by derived stage AND resolves every active spec's `## Discoveries`. After Stage 2, because an archived spec's discoveries are exactly the ones that must not be lost. |
...
`docs/` front is the handoff: the conductor hands `/specs:align` its Step 1 inventory, so
...
| --- | --- | --- |
| `sp-no-workspace`, `sp-legacy-workspace`, `sp-doctor`, `sp-invalid-plan` (mechanical) | `/specs:align` | Yes (a legacy fold's code-coupled renames gate individually) |
| `sp-plan-name`, `sp-archive-name`, `sp-task-slug` | `/specs:align` | Yes (code-coupled renames gate individually) |
| `sp-backlog-missing`, `sp-index-frontmatter`, `sp-task-untyped`, `sp-zone-missing`, `sp-zone-stale` | `/specs:align` | Yes |
| `sp-shadow-skill`, `sp-shadow-command` | `/specs:align` | Yes (migration only) |
| `sp-plan-complete` — every artifact `done`, all tasks `- [x]` | `/specs:archive` | Yes — **one OK per plan** |
| `sp-backlog-untriaged`, invalid `priority` value | `/specs:triage` | Yes |
| `sp-plan-blocked` — an artifact stuck `blocked` | *(surface → `/specs:develop`)* | **No** — revising artifacts is authoring; reported |
| `sp-plan-stale` — old `lastModified`, unfinished tasks | *(surface → `/specs:develop` / `/specs:archive` / `/specs:archive`)* | **No** — stale is not a defect; reported with its age, and abandonment is never inferred from one |
...
- **Never proposes or implements.** `/specs:develop`, `/specs:apply`,
  `/specs:from-claude`, and `/specs:explore` are per-item skills driven by
...
- **Never infers completion.** A plan is archivable only when `specs.py status` reports every
  ...
  done ([`/specs:triage`](../..//specs:triage/SKILL.md) doctrine).
- **Never abandons.** `/specs:archive` is the third exit a plan can take, and the
