...
is `publicar derivado` by default at `reference/glossary.md`; only an explicit accepted
`não publicar` row changes that default. Its route and `assets/glossary-abbreviations.md` are
generated by `cq knowledge project` from the canonical source, with `source` and `source_sha256`
stamps. Links from any projected `/.knowledge/` source are rewritten to mapped routes; links to
an explicitly unpublished home are omitted. Coverage is measured against all mandatory map rows,
not against the pages a plan happened to select.
