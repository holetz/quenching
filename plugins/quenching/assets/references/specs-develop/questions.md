...
| **Refine** | a scope or critique signal fired, and refine has not run this pass | a **raise to `high`** ([gears.md](${CLAUDE_PLUGIN_ROOT}/assets/references/specs-cycle/gears.md) §Re-evaluating a gear, the fourth signal); the choice IS the OK to restamp `complexity`, and the pass re-enters at refine |
