...
| Repo-specific term in the bundle, absent from `knowledge/glossary.md` | `/quenching:docs:glossary-backfill` | **Offered** — no cheap signal proves the gap, so the human is asked once per run |
...
| A generic concept / standard / term a human has NOT yet stated | *(surface → `/quenching:docs:learn` / `/quenching:docs:add` / `/quenching:docs:define`)* | **No** — no fresh human input this pass; reported |
