...
def compute_build(root: Path) -> BuildResult:
    ...
    brand_path = root / ".knowledge" / "standards" / "design" / "brand.md"
    production_path = root / ".knowledge" / "standards" / "design" / "production.md"
...
def _knowledge_sources(root: Path) -> list[Path]:
    ...
    if not bundle.is_dir():
        raise DesignError("PRODUCT.md projection requires an installed /.knowledge/ bundle")
    paths: set[Path] = set()
...
def _render_product(root: Path, sources: list[Path]) -> str:
    ...
    if platform not in {"web", "ios", "android", "adaptive"}:
        raise DesignError(
            ...
            "under /.knowledge/standards/platform/ or /.knowledge/vision/"
        )
...
def _generated_notice(root: Path, source: str) -> str:
    ...
    if source == "knowledge":
        return ("GERADO por `cq design build` a partir de /.knowledge/. Não edite aqui."
                if portuguese else
                "GENERATED by `cq design build` from /.knowledge/. Do not edit here.")
