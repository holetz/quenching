---
description: Align AND update every front — docs, specs, .claude — looped until nothing changes
argument-hint: [optional-scope]
allowed-tools: Read, Grep, Glob, Bash, Skill
---

# /align-and-update — every front, until nothing changes anywhere

**Input**: `$ARGUMENTS` (an optional scope; omit to drive the whole repository to a cross-front fixpoint).
...
[align-and-update-all/convergence.md](${CLAUDE_PLUGIN_ROOT}/assets/references/align-and-update-all/convergence.md): the cycle-authorization rules, the
...
- **Order is a dependency, not a preference.** `docs/` → `specs/` → `.claude/`, the same
  order `/align` uses and for the same reasons: the other two fronts write OKF
...
- **Loop across fronts, because they feed each other.** This is what separates this skill from
  ...
  - `/specs:align-and-update` **archives** a change → its distillation pass mints docs into
    `docs/` → the `docs/` front's `/docs:glossary-backfill` must now index those terms.
  - `/skill:align-and-update` **creates** the rule and registry in `docs/` → the
    `docs/` front's `index.md` and `log.md` must list them.
  - `/docs:align-and-update`'s **harness** stage moves a fact into `docs/` that the
...
- **One OK for the whole repo; authorization nests one level.** The gate fires **once**, before
  ...
  does not ask again ([convergence.md](${CLAUDE_PLUGIN_ROOT}/assets/references/align-and-update-all/convergence.md) §Nesting). After that OK, the
...
### 3. Front 1 — `/docs:align-and-update` (the `docs/` front)
Invoke via the **Skill** tool under its registry name **`claude-quenching:docs:align-and-update`**
— the command path prefixed by the plugin. Every front below is named the same way: a bare
`/docs:align-and-update` is what a human types, not what the Skill tool resolves. Declare the
authorization mode verbatim per
[convergence.md](${CLAUDE_PLUGIN_ROOT}/assets/references/align-and-update-all/convergence.md) §cycle-authorization, naming *this* conductor as the
...
### 4. Front 2 — `/specs:align-and-update` (the `specs/` front)
Skip if the front is absent and the scaffold line was declined. Otherwise invoke
**`claude-quenching:specs:align-and-update`** with the same declaration. Record its
...
### 5. Front 3 — `/skill:align-and-update` (the `.claude/` front)
Skip if the surface is empty. Otherwise invoke **`claude-quenching:skill:align-and-update`** with
the same declaration. Record its counts and
...
- Never ask for a second authorization, and never let a front conductor re-gate — authorization
  nests one level ([convergence.md](${CLAUDE_PLUGIN_ROOT}/assets/references/align-and-update-all/convergence.md) §Nesting). Equally, never suppress
...
- Never loop past the cross-front pass cap of 3, and never re-run a pass that just changed
  nothing.
- Never hand this command file `context: fork` — the gate and every nested confirmation are mid-flow.
