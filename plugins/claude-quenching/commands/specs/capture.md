---
description: Park ONE spec in the backlog inbox — fast, minimal, zero interrogation
argument-hint: [what to capture]
allowed-tools: Read, Grep, Glob, Write, Edit, Bash(python3:*), Bash(py:*)
effort: low
---

# /specs:capture — park one spec in the definition phase

**Input**: `$ARGUMENTS` (a short description of the problem to park).
...
v2 has one file: this skill creates it, `/specs:develop` fills its sections,
`/specs:apply` builds it, and it ends in `archive/` under the same basename. So there is
...
[specs-capture/backlog-zone.md](${CLAUDE_PLUGIN_ROOT}/assets/references/specs-capture/backlog-zone.md); the shared log procedure with
`/docs:add`
...
- **Capture in seconds — `## Problem` and nothing else.** `specs.py new` stamps the frontmatter
  ...
  ([spec-driven.md](${CLAUDE_PLUGIN_ROOT}/assets/references/specs-develop/spec-driven.md) §The phase gates) is what
...
- **Zero interrogation.** Never ask for scope, tasks, design, or a verification policy. What was
  not said is left out. `/specs:develop` is where the gates get walked, and
  `/specs:refine` is where the hard questions get asked — neither belongs here.
...
`okf-validate.py` per [specs-capture/backlog-zone.md](${CLAUDE_PLUGIN_ROOT}/assets/references/specs-capture/backlog-zone.md) §Resolving the tool.
...
[docs-add/homes.md](${CLAUDE_PLUGIN_ROOT}/assets/references/docs-add/homes.md). Read only the
...
coins is caught by `/docs:glossary-backfill`, or by `/docs:define` when the human
