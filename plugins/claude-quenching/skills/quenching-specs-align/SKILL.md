...
description: >-
  ...
  "make specs conformant", "clean up specs/", "migrate specs to the single-file format", or
  "migrate openspec to specs". Scaffolds the three phase folders, installs specs.py, folds a v1
  three-file workspace one-way via specs.py migrate, applies doctor/validate's DECLARED remedies,
  normalizes filenames and slugs, stamps missing frontmatter, and regenerates the backlog listing
  zone. Authoring and cycle actions are REPORTED, never driven — an empty section, a missing gate
  heading, a complete spec awaiting promote. ONE plan, one confirmation. Not for: aligning
...
- **Two migrations, both one-way, in this order.** A legacy `openspec/` workspace folds into
  `specs/` first ([conformance](references/conformance.md) §Migrating a legacy `openspec/`
  ...
  ([conformance](references/conformance.md) §Migrating a v1 workspace). The v1 fold is the tool's
...
- **Authoring is reported, never supplied — and v2 widens what that covers.** A present-but-empty
  ...
  the cycle findings in [conformance](references/conformance.md) §Findings the sweep REPORTS — a
...
**Read each checker for what it owns** ([conformance](references/conformance.md) §What checks the
