---
name: quenching-skill-eval
description: >-
  Evaluates ONE skill by running cases against it twice — with the skill loaded and without —
  in isolated subagents, grading every assertion against quoted evidence, and reporting the
  delta over pass rate, tokens, and duration. Use when the user asks to "evaluate this skill",
  "test this skill", "write evals for X", "does this skill actually help", "benchmark the
  skill", or "tune the skill's triggers". Writes evals/evals.json beside the skill, a per-case
  grading.json, and a benchmark.json carrying the with/without delta; a skill whose delta is
  zero is reported as teaching nothing, never quietly passed. Description tuning runs on
  measured should-trigger / should-not-trigger hit rates, never on taste. ONE plan for the
  cases, one OK, then the runs. Not for: minting or editing a skill → quenching-skill-new;
  migrating the whole surface → quenching-skill-align; the mechanical conformance checks →
  skills.py lint, which this never duplicates.
when_to_use: >-
  measuring whether ONE skill changes what an agent does, and tuning its description on hit
...
[references/evaluation.md](references/evaluation.md), adopted from Anthropic's `skill-creator` so
...
- **Never rewrites the body.** Authoring is `quenching-skill-new`'s. A finding here is reported
  with the `/skill:new <name>` invocation that acts on it — except the description, which step 7
...
`${CLAUDE_PLUGIN_ROOT}/assets/bin/skills.py` first, then the target's `.claude/hooks/skills.py`,
...
### 2. Read the skill as its own specification
Read the `SKILL.md` and every `references/*.md` a step points to. The description's promise —
...
### 3. Derive the cases
Per [references/evaluation.md](references/evaluation.md) §Writing cases: one case per branch, each
...
### 5. Write `evals/evals.json`, then run both arms
Write the file beside the skill. Then, per case, dispatch **two** `Task` subagents — one told the
...
- Never rewrite a skill body here — report it with its `/skill:new` invocation. The description is
...
- Never hand this `SKILL.md` `context: fork` — steps 4 and 7 gate mid-flow.
