...
description: >-
  ...
  procedure. Requires an existing OKF bundle (run quenching-align first). Not for: ONE doc
  the human states → quenching-insert; installing/aligning the docs structure →
  quenching-align; draining ~/.claude project memory → quenching-memory-to-docs.
when_to_use: >-
  importing an external source (files/folders/URLs) into an existing OKF bundle as multiple
  minted concept docs. A batch fan-out of quenching-insert; one human-stated fact is
  quenching-insert, project-memory migration is quenching-memory-to-docs.
allowed-tools: Read, Grep, Glob, WebFetch, Write, Edit, Task
---

# quenching-enrich — import an external source into the OKF bundle
...
`quenching-insert`**: [`quenching-insert/references/homes.md`](../quenching-insert/references/homes.md)
...
`quenching-align` first if `docs/` is not one.
...
## Workflow (force-with-1-confirmation, like `quenching-align`)
...
a doc (correctable), unlike `quenching-memory-to-docs` (see the model policy in
...
confirmation item (mirrors `quenching-align`) — never folded into the batch OK.
