...
description: >-
  ...
  standards/, decisions/, backlog/, or knowledge/, presents ONE migration plan executed on
  a single confirmation, and deletes a memory only after its doc lands and self-checks —
  user/unroutable memories are flagged and kept. Not for: ONE fresh fact the human states
  → quenching-knowledge; a single doc into any home → quenching-insert.
when_to_use: >-
  draining the project's ~/.claude memory files into the OKF bundle, then clearing them.
  Fresh single knowledge is quenching-knowledge; a single doc is quenching-insert.
allowed-tools: Read, Grep, Glob, Bash, Write, Edit, Task
---

# quenching-memory-to-docs — drain project memory into the OKF bundle
...
Assumes the bundle already exists (run `quenching-align` first if not). The memory-type → home routing
...
`quenching-insert` ([../quenching-insert/references/homes.md](../quenching-insert/references/homes.md)) and
`quenching-align` ([../quenching-align/references/taxonomy.md](../quenching-align/references/taxonomy.md),
[.../conformance.md](../quenching-align/references/conformance.md)). Molds live at
...
- **Four homes only.** This skill writes into exactly `standards/`, `decisions/`, `backlog/`, and
  `knowledge/` — the four "how/what WE build" concept homes. A memory whose natural fit is a
...
- **Content decides the home; `metadata.type` is a hint.** Route by what the fact IS
  ([references/memory-routing.md](references/memory-routing.md)), reusing `quenching-insert`'s home
...
- **Feed the glossary.** When a migrated memory introduces a repo-specific term, add its entry to
  ...
  step `quenching-insert`/`quenching-knowledge` run — so the term is resolvable once the doc lands.
...
### 3. Classify each → home + `type` + mold
First **orient in the bundle, bounded**: read only the four target homes' listings — never
enumerate the whole tree (`catalog/` and `reference/repositories/` will overflow the session):

```bash
for h in standards decisions backlog knowledge; do
  echo "== $h =="; cat "docs/$h/index.md" 2>/dev/null
...
hint) to its OKF home, `type`, and mold. This skill writes to **only four homes** —
`standards/`, `decisions/`, `backlog/`, `knowledge/`; a memory whose natural fit is `vision`,
`documentation`, or `reference` is **re-routed to the nearest of the four** per the routing table, and a
...
### 5. Per memory: insert, verify, then delete
For each **migrate** row, run the full insert procedure exactly as
...
- **Only after the self-check passes:** delete the memory `.md` and prune its `- [..](..)` line
  from `MEMORY.md`. A failed insert leaves that memory untouched — write-then-verify-then-delete,
...
- Never enumerate the whole bundle (`find docs -type f`) or descend into `catalog/` /
  `reference/repositories/` — inspect only the four homes' `index.md` (bounded). Never load every
  memory body into the orchestrator; recon is metadata-first, bodies are read inline (small dir) or
  by per-slice sub-agents (large dir).
