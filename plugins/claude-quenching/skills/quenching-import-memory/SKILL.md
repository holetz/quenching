...
description: >-
  ...
  standards/ or knowledge/ (in docs/), or a task in openspec/backlog/, presents ONE migration
  ...
  → quenching-learn; a single doc into any home → quenching-add.
when_to_use: >-
  draining the project's ~/.claude memory files into the OKF bundle, then clearing them.
  Fresh single knowledge is quenching-learn; a single doc is quenching-add.
allowed-tools: Read, Grep, Glob, Bash, Write, Edit, Task
user-invocable: false
---

# quenching-import-memory — drain project memory into the OKF bundle
...
`quenching-add` ([../quenching-add/references/homes.md](../quenching-add/references/homes.md)) and
...
- **Plan first, execute on one confirmation.** Cover **every** memory, classify each, and present
  ...
  invoked by `quenching-converge` under its cycle-authorization contract
...
- **Three destinations only.** This skill writes into exactly two `docs/` homes — `standards/`
  and `knowledge/` — plus `openspec/backlog/` for a **task** (a quenching-managed folder outside
...
- **Content decides the home; `metadata.type` is a hint.** Route by what the fact IS
  ([references/memory-routing.md](references/memory-routing.md)), reusing `quenching-add`'s home
...
- **Feed the glossary.** When a migrated memory introduces a repo-specific term, add its entry to
  ...
  step `quenching-add`/`quenching-learn` run — so the term is resolvable once the doc lands.
...
echo "== openspec/backlog =="; cat "openspec/backlog/index.md" 2>/dev/null
```
...
`knowledge/` (in `docs/`) plus `openspec/backlog/` (a `task`); a memory whose natural fit is `vision`,
...
- A **task** row instead follows the `openspec-backlog` capture path: stamp the `task.md` mold
  into `openspec/backlog/<slug>.md`, regenerate its DERIVED zone, and run the on-write self-check
  per [../openspec-backlog/references/backlog-zone.md](../openspec-backlog/references/backlog-zone.md)
  (the OKF hook does not cover the backlog); the bundle-log line is
  `**Creation**: [<title>](/openspec/backlog/<slug>.md) — migrated from project memory`.
...
- Never write outside the three destinations (`standards/` + `knowledge/` in `docs/`, or a task
  in `openspec/backlog/`) — re-route to the nearest, or flag-and-keep; never fabricate a
