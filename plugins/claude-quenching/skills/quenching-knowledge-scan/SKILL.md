...
description: >-
  ...
  capturing ONE fresh concept → quenching-knowledge; a single on-demand term →
  quenching-glossary.
when_to_use: >-
  ...
  term on demand is quenching-glossary.
...
`quenching-knowledge`'s job. The glossary format and the shared **Enriching the glossary**
procedure live with `quenching-insert`
...
`quenching-memory-to-docs`, the only other skill in this plugin that uses `Task`.
...
- **Sweep, don't recapture.** A retroactive, bulk pass over what the bundle *already says* —
  ...
  stays exclusively `quenching-knowledge`'s job).
- **Bounded reconnaissance, sliced fan-out.** List doc paths cheaply first (`Glob`/`find`, no
  ...
  to the orchestrator. Apply the same caution `quenching-memory-to-docs` applies to
...
- **MERGE, never clobber; keep it sorted; skip what's already listed** — identical contract
  to `quenching-glossary` Step 4, in the bullet syntax from
...
- Never author a `knowledge/` concept doc or ask the human what a term means — that is
  `quenching-knowledge`'s job, not this skill's.
