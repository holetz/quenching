...
description: >-
  ...
  tool/lib fact → quenching-add; draining project memory → quenching-import-memory.
when_to_use: >-
  filing ONE piece of generic understanding into knowledge/. Contracts, decisions, documentation,
  and external-dependency facts route via quenching-add; memory migration is
  quenching-import-memory.
allowed-tools: Read, Grep, Glob, Write, Edit
user-invocable: false
---

# quenching-learn — capture generic knowledge, OKF-conformant
...
index/log procedure are shared with `quenching-add`
([../quenching-add/references/homes.md](../quenching-add/references/homes.md)); the `type` vocabulary
...
- **`knowledge/` is generic understanding, not a contract.** It holds what the team
  ...
  **stop and route via `quenching-add`** rather than mis-filing it under `knowledge/`.
...
- **Feed the glossary.** `knowledge/` ships one fixed file — `glossary.md`, the repo's A–Z term
  ...
  `quenching-glossary-backfill` is the whole-bundle counterpart that backfills terms already sitting
...
to `quenching-add`. Otherwise proceed under `knowledge/`.
...
[../quenching-add/references/homes.md](../quenching-add/references/homes.md) (the tail step
every capture runs; on-demand counterpart `quenching-define`, bulk counterpart
`quenching-glossary-backfill`).
...
- Never file a **contract / decision / procedure / external-asset fact** under `knowledge/` —
  route it to its home via `quenching-add`.
