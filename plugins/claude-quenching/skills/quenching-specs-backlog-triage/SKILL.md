---
name: quenching-specs-backlog-triage
description: >-
...
themes for what is parked there — the sweep counterpart of the per-item `quenching-specs-backlog-add`
...
- **Completion is stated, never inferred.** A removal + Completed-ledger row enters the
  plan only when the human says the task is done or its plan is apply-ready.
...
stop and offer `quenching-specs-backlog-add` (which installs the seed) or note there is nothing
...
link to the resulting plan/PR/standard or a one-line "done — …".
...
the Completed ledger holds only approved rows. The hook is docs-scoped by config, so it does not
