...
leaves the OKF `type` vocabulary). Create `specs/backlog/` — `quenching-specs-align` scaffolds the
...
`quenching-docs-align-and-update` under its cycle-authorization contract
([convergence.md §contract](../../quenching-align-and-update-all/references/convergence.md)) replaces only the batch gate
