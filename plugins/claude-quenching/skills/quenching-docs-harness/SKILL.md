---
name: quenching-docs-harness
description: >-
  Refactors a repo's harness files — the root CLAUDE.md, every subfolder CLAUDE.md, and
  AGENTS.md — into thin, honest navigation pointers over the OKF docs/ bundle, MOVING
  (never copying) inlined durable knowledge into its home. Use when the user asks to
  "refactor CLAUDE.md", "slim down CLAUDE.md", "move CLAUDE.md content into docs", "make
  CLAUDE.md point to the knowledge base", "align CLAUDE.md/AGENTS.md with docs/", or
  "clean up the agent instructions". Classifies every content unit (keep the operational,
...
# quenching-docs-harness — make CLAUDE.md a thin, honest pointer into the bundle
...
vocabulary, molds, and index/log procedure are shared with `quenching-docs-add`
...
[../quenching-docs-align/references/migration.md](../quenching-docs-align/references/migration.md)). Harness
...
- **Plan first, execute on one confirmation.** Read every harness file, classify every unit, and
  ...
  `quenching-docs-import-memory` ran — cycle.md §Parallel prep); before writing, re-verify any unit
...
gated item — never auto-created. See [references/harness-routing.md](references/harness-routing.md) §6.
...
[../quenching-docs-add/references/homes.md](../quenching-docs-add/references/homes.md); a **DEDUPE**
...
([../quenching-docs-align/references/migration.md](../quenching-docs-align/references/migration.md) §3–4):
