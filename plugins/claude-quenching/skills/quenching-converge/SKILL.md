---
name: quenching-converge
description: >-
  Drives a repository's knowledge surface to OKF convergence by running the plugin's
  sweep skills — quenching-align, then quenching-import-memory, then quenching-harness,
  then quenching-glossary-backfill — as a dependency pipeline, pass after pass, until a full
...
# quenching-converge — drive the repo to OKF convergence, pass by pass
...
- **Convergence to a fixpoint, not a single run.** Inherits the plugin's *convergence, not
  ...
  variants, `quenching-import-memory` finds no undrained memory, `quenching-harness` finds a thin
  harness, `quenching-glossary-backfill` finds no unlisted terms — so a pass where **every** stage
...
- **Conduct, never reimplement.** The cycle sequences and gates; it does not re-derive a stage's
  ...
  authority per concern — the same reason `quenching-add` delegates the glossary tail step
...
- **Order is a dependency pipeline.** Structure → content → glossary → validation:
  ...
  content feeders `quenching-import-memory` and `quenching-harness` (they move facts in from the
  two out-of-band stores — project memory and the harness files), then `quenching-glossary-backfill`
  (a glossary swept before content lands misses terms), then re-validate. Never run a later stage
  before an earlier one.
- **Per-item skills are stage tools, not stages.** `quenching-add` / `quenching-learn` /
  `quenching-define` each act on **one** item a human states, and the cycle has **no** fresh
  human input per pass — so they are not loop stages. They are the tools the sweep stages already
  delegate to (`quenching-harness` MOVEs a durable fact via `quenching-add`; a coverage-ledger
  deferral is an `quenching-add` opportunity). The cycle **surfaces** such content gaps in its
...
2. `quenching-import-memory` — drain project memory into its homes, clear each migrated memory.
3. `quenching-harness` — thin `CLAUDE.md`/`AGENTS.md`, MOVE durable knowledge into homes.
4. `quenching-glossary-backfill` — backfill `knowledge/glossary.md` from the whole bundle.
...
- **Pass changed something** → run the next pass (back to Step 4) under the same authorization —
  ...
  `quenching-glossary-backfill` should now index; `quenching-align` relocated a doc that shifted an
