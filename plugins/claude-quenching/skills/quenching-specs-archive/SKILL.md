---
name: quenching-specs-archive
description: >-
  Closes a spec out — promotes it to archive/ with `outcome: done` or `outcome: abandoned`, then
  offers ONE OKF distillation pass for what the work left behind. Use when the user asks to
  "archive this", "close it out", "it's done, wrap it up", "finalize the spec", or — for work
  that will NOT be built — "drop this", "abandon it", "we're not doing this". The outcome decides
  everything downstream: done distils by-products into docs/ (a proven rule -> standards/
  authority-graded, an understanding -> knowledge/, a term -> glossary) and REFUSES while boxes
  are still open unless forced; abandoned distils nothing as adopted — at most a background note
  on what was learned by NOT building it — and is always allowed, because open tasks are exactly
  what you expect there. Never inferred from staleness by any sweep. Not for: building the
  remaining tasks -> quenching-specs-apply; a doc straight into docs/ -> quenching-docs-add.
when_to_use: >-
  closing a spec out — promote to archive/ as done or abandoned, then the distillation offer.
allowed-tools: Bash(python3:*), Bash(py:*), Read, Glob, Grep, Write, Edit, AskUserQuestion, Skill
user-invocable: false
---

# quenching-specs-archive — close a spec out, then distil
...
[references/distill.md](references/distill.md). The layout, the gates, and the `specs.py` surface
live in
...
Per [references/distill.md](references/distill.md), and **branching on the outcome**:
...
[../quenching-docs-add/references/homes.md](../quenching-docs-add/references/homes.md). No OKF
...
are `quenching-specs-triage`'s to resolve, and they are easy to lose at close.
