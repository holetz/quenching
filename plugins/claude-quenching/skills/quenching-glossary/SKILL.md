...
description: >-
  ...
  entry, and logs the update. Not for: a full concept doc → quenching-knowledge;
  whole-bundle term backfill → quenching-knowledge-scan.
when_to_use: >-
  adding or refining ONE term entry in knowledge/glossary.md. A full concept doc is
  quenching-knowledge; a bundle-wide backfill sweep is quenching-knowledge-scan.
...
glossary format, and the shared **Enriching the glossary** procedure live with `quenching-insert`
([../quenching-insert/references/homes.md](../quenching-insert/references/homes.md)); the `type`
...
backfill instead of one term, see `quenching-knowledge-scan`.
...
- **The glossary is an index, not a home for depth.** An entry is a **one-sentence** meaning plus
  ...
  sentence, the depth belongs in a `knowledge/` concept doc (`quenching-knowledge`) and the entry
...
than a one-liner, say so and hand off to `quenching-knowledge` for the concept doc — then add the
...
note that a `quenching-knowledge` capture would give the term a home.
