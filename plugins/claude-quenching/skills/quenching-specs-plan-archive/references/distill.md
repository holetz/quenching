...
How `quenching-specs-plan-archive` offers to carry the **durable** knowledge a completed plan
...
| --- | --- | --- |
| the **behavior/rule the plan implemented** | **no** | the plan **already wrote it into `docs/standards/`** as it was built (`quenching-specs-plan-apply`) — nothing to do here. There is no separate spec store to sync |
| a **decision** (rationale + considered alternatives, usually in `design.md`) not yet captured as a standard | yes | `standards/<subject>/<concept>.md` (`type: standard`; `authority: current` if the plan proved it, else `background`) — there is no separate ADR home |
...
| a **follow-up task** the plan surfaced but did not pursue | yes | `specs/backlog/<task-slug>.md` (`type: task`, task mold — untriaged unless the human states a priority; via `quenching-specs-backlog-add`, outside the `docs/` bundle) |
| proposal.md / design.md / tasks.md as documents | no | they are the archive's history; copying them into `docs/` duplicates a source of truth |
...
behavior* — it is to catch the **by-products** (a decision left in `design.md`, an understanding, a
...
1. **Harvest candidates.** Read the archived plan's artifacts (`proposal.md`, `design.md`,
   `tasks.md`) and the conversation's own context, and check what `docs/standards/` already
...
3. **Mint each approved doc** under the insert procedure
   ...
   term warrants it. A follow-up **task** instead uses the `quenching-specs-backlog-add` capture
