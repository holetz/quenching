...
# quenching-docs-align-and-update — align AND update the `docs/` front, pass by pass

The **`docs/` front's conductor**, one of three (`quenching-specs-align-and-update` and
`quenching-skill-align-and-update` are its peers; `quenching-align-and-update-all` runs all
three). The name says what separates it from `quenching-docs-align`: align fixes **structure** in
...
with `quenching-docs-align` ([../quenching-docs-align/references/taxonomy.md](../quenching-docs-align/references/taxonomy.md),
[.../conformance.md](../quenching-docs-align/references/conformance.md)); the executable checker is
...
- **Convergence to a fixpoint, not a single run.** Inherits the plugin's *convergence, not
  ...
  count of passes. Every sub-skill is idempotent on a converged repo — `quenching-docs-align` finds no
  variants, `quenching-docs-import-memory` finds no undrained memory, `quenching-docs-harness` finds a thin
  harness, `quenching-docs-glossary-backfill` finds no unlisted terms — so a pass where **every** stage
...
- **Conduct, never reimplement.** The cycle sequences and gates; it does not re-derive a stage's
  ...
  authority per concern — the same reason `quenching-docs-add` delegates the glossary tail step
...
- **One OK per cycle; the cycle run is the unit.** The gate runs ONCE, before Pass 1: present
  ...
  [../quenching-align-and-update-all/references/convergence.md](../quenching-align-and-update-all/references/convergence.md) — later passes run without a gate, narrating their
  plan. The authorization absorbs each stage's routine plan pause; it never absorbs a
  **code-coupled** item: a rename inside `quenching-docs-align`/`quenching-docs-harness` whose blast
...
- **Order is a dependency pipeline.** Structure → content → glossary → validation:
  `quenching-docs-align` first (nothing can be listed into a tree that isn't there), then the two
  content feeders `quenching-docs-import-memory` and `quenching-docs-harness` (they move facts in from the
  two out-of-band stores — project memory and the harness files), then `quenching-docs-glossary-backfill`
  (a glossary swept before content lands misses terms), then re-validate. Never run a later stage
  before an earlier one.
- **Per-item skills are stage tools, not stages.** `quenching-docs-add` / `quenching-docs-learn` /
  `quenching-docs-define` each act on **one** item a human states, and the cycle has **no** fresh
  human input per pass — so they are not loop stages. They are the tools the sweep stages already
  delegate to (`quenching-docs-harness` MOVEs a durable fact via `quenching-docs-add`; a coverage-ledger
  deferral is an `quenching-docs-add` opportunity). The cycle **surfaces** such content gaps in its
...
`log.md` / `okf_version`), that is expected — the first pass's `quenching-docs-align` stage installs it.
...
([convergence.md](../quenching-align-and-update-all/references/convergence.md)) for the whole run; it does **not** pre-authorize
...
([convergence.md](../quenching-align-and-update-all/references/convergence.md) §cycle-authorization). When BOTH content feeders (stages 2 and
...
1. `quenching-docs-align` — converge structure, regenerate listings, validate.
2. `quenching-docs-import-memory` — drain project memory into its homes, clear each migrated memory.
3. `quenching-docs-harness` — thin `CLAUDE.md`/`AGENTS.md`, MOVE durable knowledge into homes.
4. `quenching-docs-glossary-backfill` — backfill `knowledge/glossary.md` from the whole bundle.
...
- **Pass changed something** → run the next pass (back to Step 4) under the same authorization —
  ...
  surface **follow-on** work the prior pass created (e.g., `quenching-docs-harness` MOVEd a fact that
  `quenching-docs-glossary-backfill` should now index; `quenching-docs-align` relocated a doc that shifted an
...
against [../quenching-docs-align/references/conformance.md](../quenching-docs-align/references/conformance.md).
