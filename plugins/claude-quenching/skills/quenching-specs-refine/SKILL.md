---
name: quenching-specs-refine
description: >-
  Interrogates a spec until it is worth building — generating the questions nobody thought to
  ask, ONE at a time with an inline recommendation, applying every answer in a SINGLE edit at
  the end. Use when the user asks to "refine the spec", "challenge this", "poke holes in it",
  "what am I missing here", "critique the design", "run a premortem", "what alternatives did we
  skip", or "interview me about this". Four modes select the technique — interview (default),
  critic, premortem, alternatives — each with a stop condition, so it terminates instead of
  wandering. Writes answers back through specs.py section and records refined {mode, date} in
  the spec's frontmatter, which clears validate's sp-unrefined. Never edits code, never writes
  into docs/, and never gates: an unrefined spec may always be promoted. Not for: applying an
  edit the human already formulated -> quenching-specs-develop (reactive, where this is
  generative); open-ended thinking before a spec exists -> quenching-specs-explore.
when_to_use: >-
  interrogating a spec to surface what nobody asked, then applying the answers in one edit.
allowed-tools: Bash(python3:*), Bash(py:*), Read, Glob, Grep, Write, Edit, AskUserQuestion
user-invocable: false
---

# quenching-specs-refine — interrogate a plan until it is worth building
...
`quenching-specs-develop` waits for an edit the human already formulated and reconciles it;
...
[../quenching-specs-develop/references/artifacts.md](../quenching-specs-develop/references/artifacts.md).
...
[artifacts.md](../quenching-specs-develop/references/artifacts.md) — an emptied section
...
- **Never edits code, never writes into `docs/`.** This skill touches a plan's three artifacts and
  ...
  `quenching-docs-define`, an out-of-scope follow-up to `quenching-specs-capture` — **offered,
  never auto-written**.
