...
How `quenching-add` decides **where** a new concept goes, **which** `type` it carries, **which**
...
every skill that inserts a doc (`quenching-add`, `quenching-learn`, `quenching-harness`,
`quenching-import-memory`) cites these sections instead of restating them; only each skill's own
...
- **standards & agreed rules:** a **proven, current** rule for how we build is a `standard`
  ...
  rationale and considered alternatives are captured in an OpenSpec change's `design.md` while
  the change is active, and its durable outcome lands as a `standard` at archive time.
- **task vs vision:** a **parked unit of work** is a `task` — but the backlog lives at
  `openspec/backlog/`, **outside** this OKF bundle, so a task is captured with
  `openspec-backlog` (not inserted here); a **settled direction** with no deadline is a
  `vision`. A raw task graduates to a `vision`/`standard` only after the OpenSpec cycle
  (`openspec-explore` / `openspec-propose`) develops it into a change with apply-ready
  artifacts; on completion/approval the task file leaves `openspec/backlog/` (its Completed
...
(The `openspec/backlog/index.md` DERIVED zone is **not** part of this OKF insert procedure —
...
This is the same tail step `quenching-learn`, `quenching-add`, and
`quenching-import-memory` each run; the on-demand single-term counterpart is the
`quenching-define` skill, and the whole-bundle bulk counterpart — sweeping every doc
already in `docs/` for terms the glossary never caught — is `quenching-glossary-backfill`.
