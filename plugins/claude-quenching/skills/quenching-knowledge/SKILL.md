...
description: >-
  ...
  self-checks. Not for: a rule for how WE build, a decision, a how-to, or an external
  tool/lib fact → quenching-insert; draining project memory → quenching-memory-to-docs.
when_to_use: >-
  filing ONE piece of generic understanding into knowledge/. Contracts, decisions, documentation,
  and external-dependency facts route via quenching-insert; memory migration is
  quenching-memory-to-docs.
allowed-tools: Read, Grep, Glob, Write, Edit
---

# quenching-knowledge — capture generic knowledge, OKF-conformant
...
Assumes the bundle already exists (run `quenching-align` first if not). The mold lives at
`${CLAUDE_PLUGIN_ROOT}/assets/templates/concept-front.md`; the home boundaries and the
index/log procedure are shared with `quenching-insert`
([../quenching-insert/references/homes.md](../quenching-insert/references/homes.md)); the `type` vocabulary
and conformance rules with `quenching-align`
([../quenching-align/references/taxonomy.md](../quenching-align/references/taxonomy.md),
[.../conformance.md](../quenching-align/references/conformance.md)).
...
- **`knowledge/` is generic understanding, not a contract.** It holds what the team
  ...
  **WE** do it" it is a `standard`; an open decision is an ADR; a set of steps for using the product is `documentation`; a
  fact about a **named** external tool/lib/regulation is `reference`. When it belongs elsewhere,
  **stop and route via `quenching-insert`** rather than mis-filing it under `knowledge/`.
...
- **Feed the glossary.** `knowledge/` ships one fixed file — `glossary.md`, the repo's A–Z term
  ...
  `quenching-knowledge-scan` is the whole-bundle counterpart that backfills terms already sitting
...
to `quenching-insert`. Otherwise proceed under `knowledge/`.
...
[../quenching-insert/references/homes.md](../quenching-insert/references/homes.md), with the entry:
...
[../quenching-insert/references/homes.md](../quenching-insert/references/homes.md) (the tail step
every capture runs; on-demand counterpart `quenching-glossary`, bulk counterpart
...
- Never file a **contract / decision / procedure / external-asset fact** under `knowledge/` —
  route it to its home via `quenching-insert`.
