...
The ingestion-specific detail `quenching-import` owns. The **per-doc** procedure (classify →
stamp → index → log → glossary → self-check) is **not** here — it lives once in
[`quenching-add/references/homes.md`](../../quenching-add/references/homes.md), which
...
Out of scope (route elsewhere): ONE fact the human states → `quenching-add`; the project's
`~/.claude` memory → `quenching-import-memory`; installing/aligning the tree →
`quenching-align`.
...
correctable move), unlike `quenching-import-memory` where a misclassification deletes a
