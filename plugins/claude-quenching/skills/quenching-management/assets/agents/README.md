...
| `quenching-direction.md` | sub-agent (worker) | `.claude/agents/` of target | 3/10/12 DIRECTION-DRAFT: one per human-direction dim (vision/memory/boundary); seeds a draft from git history/README/backlog/ADR and writes it `authority: background` + "DRAFT — pending human ratification"; **never** `current`, the human ratifies |
...
nothing. `quenching-writer` (derived `current` standards) and
`quenching-direction` (labeled, ratification-gated direction drafts) are the two
**writing** disciplines — the inverse regimes of the apply model.

> **Method maintainer tooling is NOT here.** The **skill** that develops the
> **method itself** (`quenching-maintainer`) is **maintainer tooling** and, by
> doctrine, is **never installed into a target repo**. It lives outside this shipped
> package, at the repository root (`.claude/skills/`), and is documented in the
> project's `CONTRIBUTING.md`.

## How to install (summary)

1. **Copy** the workers (`quenching-auditor.md`, `quenching-writer.md`,
   `quenching-direction.md`) to `.claude/agents/` of the target.
2. **Rename** `name`/file to the derived taxonomy (`<prefix>`); the `description`
   is **agent routing code → keep it English** (dim 6/7).
