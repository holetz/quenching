...
> (`quenching-auditor`, `quenching-writer`, `quenching-direction`). The **skill**
> that develops the **method itself** (`quenching-maintainer`) is **maintainer
> tooling**: it is **never** shipped or installed, lives at the development repo's
> root `.claude/skills/`, and is documented in that repo's
...
| `agents/quenching-direction.md` | sub-agent | `.claude/agents/` | 3/10/12 DIRECTION-DRAFT (one per human-direction dim; seeds a draft from git history/README/backlog/ADR, writes it `authority: background` + "pending ratification" banner; never `current`, human ratifies) |
...
| `hooks/guard-config-surface.py` | hook (PostToolUse, matcher `Write\|Edit`) | `.claude/hooks/` + `settings.json` | 8 (config-surface guard) + 6/7 + core (Step 8) — when a Write/Edit touches the **harness itself** (`.claude/` skills/agents/hooks/settings), re-applies the cheap dims-6/7/8 checks (frontmatter present, `name`==folder, description-as-trigger, orphan/dangling wiring 8b) and only PROPOSES via `additionalContext` (exit 0, never blocks); sibling of `propose-docs-home` (that one covers `docs/`) — together they close the event trigger (1) of Step 8 |
...
| `templates/**` (13 molds — see [`templates/README.md`](templates/README.md)) | templates | stamped where missing (header: skill/agent/CLAUDE.md/ADR/backlog/memory/docs-front; receipt: quenching-manifest) or applied by `quenching-docs` (body: sidecar/area/catalog) | 1–7, 10–13 + core (lifecycle) |
| `plugin/` (README + `plugin.template.json` + `hooks.template.json` + `settings.marketplace.snippet.json`) | rollout scaffold | org marketplace repo (`<plugin>/.claude-plugin/plugin.json`, `<plugin>/hooks/hooks.json`) + each target's `.claude/settings.json` | core (lifecycle §4: `plugin` distribution channel — auto-wired hooks via `${CLAUDE_PLUGIN_ROOT}`, upgrades by `version` bump, adoption via `extraKnownMarketplaces`/`enabledPlugins`) |
| `docs/README.md` | skeleton | target's `docs/` root (canonical taxonomy manifest) | 2, 12 |
| `docs/standards/` (README + INDEX + **CLAUDE.md** + 9 subtopics) | skeleton | target's current reference (`architecture/code/naming/data-modeling/ci-cd/workflows/mlops/quality/platform`); the folder `CLAUDE.md` is a thin auto-loaded pointer (agent-facing) beside the human `README.md` | 2, 12 |
...
2. **Rename** folder/`name` to the derived taxonomy (`<prefix>`); the payload is an
   agent-facing surface — keep it **English** (only `audience: human` docs follow
   the repo's language).
