...
| `guard-config-surface.py` | PostToolUse (`Write\|Edit`) | `.claude/hooks/` + `settings.json` | 8 + 6/7 + core — when a Write/Edit touches the **harness itself** (`.claude/` skills/agents/hooks/settings), re-applies the cheap dims-6/7/8 checks (frontmatter, `name`==folder, description-as-trigger, orphan/dangling wiring 8b) and **PROPOSES** via `additionalContext`; sibling of `propose-docs-home` (that one covers `docs/`); never blocks |
...
| `run-validation.py` | Stop | `.claude/hooks/` + `settings.json` | 8 (thin-hook-calls-script, crosses `scripts/`) + 14 — at end of turn **CALLS the target-derived validation script** (`validateScript`/`validateCmd`; empty = inert) on altered files; default **PROPOSES** the result via `additionalContext` (exit 0), `blockOnFail:true` makes it mandatory |
