...
| Catalog/domain without generated×curated separation, or missing consumption doctrine | skill `quenching-docs` | target's domain layer |
...
3. **Repoint** the payload's internal paths to where the layer lives in the
   target (e.g.: the skill `quenching-docs` references "the normative layer" — fix
...
- **3. Direction (`docs/vision/`)** — proposes the edit/segmentation of the
...
- **12. Boundary doctrine** — proposes which is the canonical home and what
  ...
  that own the touched artifacts (moving a standard from VISION to normative
  reference = `quenching-docs` on the writing end).
