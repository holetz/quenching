...
description: >-
  ...
  capturing ONE fresh concept → quenching-learn; a single on-demand term →
  quenching-define.
when_to_use: >-
  ...
  term on demand is quenching-define.
...
written before the glossary existed, migrated in by `quenching-align`, or hand-authored — for
...
`quenching-learn`'s job. The glossary format and the shared **Enriching the glossary**
procedure live with `quenching-add`
...
[.../conformance.md](../quenching-align/references/conformance.md)). The fan-out convention
(bounded recon, sliced `Task` agents, compact partials, one merged plan) follows
`quenching-import-memory`, the only other skill in this plugin that uses `Task`.
...
- **Sweep, don't recapture.** A retroactive, bulk pass over what the bundle *already says* —
  ...
  stays exclusively `quenching-learn`'s job).
- **Bounded reconnaissance, sliced fan-out.** List doc paths cheaply first (`Glob`/`find`, no
  ...
  to the orchestrator. Apply the same caution `quenching-import-memory` applies to
...
- **Plan first, one confirmation.** Merge every slice into ONE consolidated list before any
  write (mirrors `quenching-align`/`quenching-import-memory`'s posture). **Exception —
  cycle-authorized runs:** invoked by `quenching-converge` under its cycle-authorization contract
  ([../quenching-converge/references/cycle.md](../quenching-converge/references/cycle.md)), the
...
- **MERGE, never clobber; keep it sorted; skip what's already listed** — identical contract
  to `quenching-define` Step 4, in the bullet syntax from
...
`quenching-align` to install the skeleton, then resume. Read the current entries once — this
...
- Never author a `knowledge/` concept doc or ask the human what a term means — that is
  `quenching-learn`'s job, not this skill's.
