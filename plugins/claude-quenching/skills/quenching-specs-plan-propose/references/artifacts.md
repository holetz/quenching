...
`docs/standards/` while it is built (`quenching-specs-plan-apply`); the proposal *declares* that
...
always exists, and a section with nothing in it is answered `- none` (in `design.md`, `- none —
...
`sp-impact-uncovered` (warn) for any `docs/standards/**.md` path bulleted there that no `tasks.md`
...
## `design.md` — required-with-explicit-fallback, depends on `proposal`

**The file always exists.** Do not delete it to signal "no design was needed": an absent
`design.md` cannot distinguish *we weighed the alternatives and there were none* from *nobody ever
thought about it*, and those are opposite facts. Answer an empty section `- none — <reason>`.

`design.md` is required as a **section set**, never as a **dependency** — `applyRequires` stays
...
next` names it as the artifact to write while `tasks.md` is still unwritten.
...
- **`## Open Decisions`** — what is deliberately still undecided, and **how each gets decided** —
  ...
  one (`- [ ] 6.5 Decide per design.md §Open Decisions whether …`).
...
`quenching-specs-plan-apply` consumes them; `CHECKBOX_RE` is untouched, so every `tasks.md`
...
`per-task` / `per-section` (default) / `end-of-plan`, recorded in `.specs.json` by
`specs.py new --verification` at propose time. It answers *when* the checks run; `verify:` answers
*what* runs. Deciding it here is what keeps `quenching-specs-plan-apply` from having to guess, or
