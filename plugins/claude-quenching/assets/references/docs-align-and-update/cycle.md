...
The reference `/docs:align-and-update` reads. It fixes the two things specific to the
...
| --- | --- | --- | --- |
| 1 | `/docs:align` | **structure** | Nothing can be filed, listed, or validated into a tree that isn't there. Align scaffolds homes, migrates variants, stamps frontmatter, regenerates every `index.md`, and runs the checker. Everything downstream assumes an aligned bundle. |
...
| 4 | `/docs:glossary-backfill` | **glossary from the whole bundle** | Now the bundle is structurally sound and both feeders have landed their content, sweep the **complete** bundle for repo-specific terms and backfill `knowledge/glossary.md`. Swept earlier, it would miss terms Stages 2–3 were still importing. |
...
1. Stage 1 (`/docs:align`) completes first, as always.
2. The cycle dispatches **one read-only `Task` sub-agent in the background** (`model: sonnet` —
   ...
   instructed to execute steps 1–4 of `/docs:harness/SKILL.md` **as written there**
...
3. In parallel, the orchestrator runs `/docs:import-memory` to completion, inline.
4. The cycle then invokes `/docs:harness`, handing it the pre-collected table **plus the
...
| --- | --- | --- |
| Variant folder name (`docs/arquitetura/`), missing applicable home | `/docs:align` | Yes |
| Un-stamped / mis-stamped frontmatter, `summary:`→`description:`, enum drift | `/docs:align` | Yes |
| Prefix-cluster to fold (`nomenclatura-*.md`), non-English slug to translate | `/docs:align` | Yes |
| `dir-no-index` / `index-broken-link` / `index-orphan` (validator WARN) | `/docs:align` | Yes |
...
| Repo-specific term in the bundle, absent from `knowledge/glossary.md` | `/docs:glossary-backfill` | Yes |
...
| A generic concept / standard / term a human has NOT yet stated | *(surface → `/docs:learn`/`-insert`/`-glossary`)* | **No** — no fresh human input this pass; reported |
