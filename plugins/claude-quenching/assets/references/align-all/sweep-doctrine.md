...
(`specs/`), and `/skill:align` (`.claude/`). They converge different artifacts, but
...
owned by `/align-and-update`, and the one place the two touch (a cycle-authorized
...
| `specs/` | `/specs:align` | `specs.py doctor` + `specs.py validate`, plus `okf-validate.py specs/backlog --listing-root` for the inbox |
