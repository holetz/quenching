...
The owner of **how** `/specs:refine` interrogates a plan. The skill body owns the
...
one more question is `/specs:explore` wearing the wrong name.
...
- a **mitigation task** in `## Tasks`;
- a **risk with a named mitigation** in `## Design` `## Risks`;
...
lands in `## Design` `## Alternatives Considered` — **including the rejected ones and why they
