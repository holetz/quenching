...
How `/specs:archive` offers to carry the **durable** knowledge a completed plan
...
mechanics (stamp → index → log → glossary → self-check) live with `/docs:add`
...
| --- | --- | --- |
| the **behavior/rule the plan implemented** | **no** | the plan **already wrote it into `docs/standards/`** as it was built (`/specs:apply`) — nothing to do here. There is no separate spec store to sync |
...
| a **follow-up task** the plan surfaced but did not pursue | yes | `specs/backlog/<task-slug>.md` (`type: task`, task mold — untriaged unless the human states a priority; via `/specs:capture`, outside the `docs/` bundle) |
...
2. **One plan, one OK.** Present the candidate list as a single distillation plan and ask one
   ...
   `okf_version`), offer `/docs:align` first, or skip distillation entirely — never
   scaffold ad hoc.
3. **Mint each approved doc** under the insert procedure
   ...
   term warrants it. A follow-up **task** instead uses the `/specs:capture` capture
