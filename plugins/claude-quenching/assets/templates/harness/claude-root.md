...
- [docs/decisions/](docs/decisions/index.md) — open decisions (ADRs).
...
<!-- HOW TO FIND A TERM: the glossary is the repo's A–Z vocabulary index at
     ...
     defined — add it with the `quenching-glossary` skill (one term), backfill many at once
     with `quenching-knowledge-scan`, or capture the concept with `quenching-knowledge` (which
     enters the term as its tail step). -->

To **create / edit / move** knowledge (keeping the listing + `log.md` in sync), use the
`claude-quenching` skills: `quenching-insert` to add one, `quenching-glossary` to add a glossary
...
<!-- MOLD (claude-quenching · root harness pointer — do NOT copy this note into the produced file):
     ...
     • KEEP only what the agent needs on EVERY task (commands, env, etiquette). MOVE durable
       knowledge into its docs/ home (via the quenching-insert procedure) and leave a one-line
