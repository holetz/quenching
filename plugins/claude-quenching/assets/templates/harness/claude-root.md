...
`claude-quenching` skills: `quenching-docs-add` to add one, `quenching-docs-define` to add a glossary
...
<!-- MOLD (claude-quenching · root harness pointer — do NOT copy this note into the produced file):
     ...
     • KEEP only what the agent needs on EVERY task (commands, env, etiquette). MOVE durable
       knowledge into its docs/ home (via the quenching-docs-add procedure) and leave a one-line
