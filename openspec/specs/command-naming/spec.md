# command-naming Specification

## Purpose

Defines how the plugin's command surface is named and partitioned: verb-first command names
that reveal their action in the `/` menu, namespaces honest to the artifact each command
touches (`opsx:`, `docs:`, `skill:`), preservation of the `opsx:` namespace and the six
upstream `openspec-*` skill names, the one-skill-one-wrapper bijection, and a clean rename
