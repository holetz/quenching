...
description: The signature /docs/ tree every adopted repository shares — its homes, the strict rules that keep it greppable, and why sameness is the feature.
...
audience: both
authority: current
source: plugins/quenching/README.md §The signature; /docs/ home index pages of this repository
...
`/docs/`, the same tree everywhere — adapted only by omission (a repo without data gets
...
- **The glossary is one file, at the root.** The single deliberate exception to "one concept
  ...
  published site ([glossary.md](../glossary.md)) with `<abbr>` definitions
  on every page.

The validator enforces what prose cannot: `cq knowledge validate docs` reports missing
...
naming standard?" with one glob (`/docs/standards/naming/`), resolve any unfamiliar term
...
!!! abstract "TL;DR for agents"
    - Root: `/docs/index.md` with `okf_version: "0.1"`; term lookup at
      `/docs/glossary.md`; homes: `standards/ concepts/ external/ documentation/
      catalog/ vision/`.
    ...
    - Verify: `cq knowledge validate docs` — exit `0` and "0 error(s)" is the gate.
