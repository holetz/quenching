...
**Update**: [Align surface](/docs/standards/architecture/align-surface.md) — the probe-before-inventory rule gains the one call that is deliberately **cross-front**: `skills.py drift` answers all three installed copies and the hook's wiring at once, so each align reads its own row instead of hand-comparing a `--version` in prose, and any one align can report the other two fronts' drift without a second probe. Recorded here because it is a **read** — the constant is parsed, never the script executed — which is what keeps a probe that runs on every align from being a probe that runs whatever a target has on disk. The doc had gone `stale-doc` the moment the four align bodies changed, its `resource` naming exactly those files; closed by writing the missing rule rather than by moving the timestamp.

**Update**: [Versioning and release](/docs/standards/ci-cd/versioning-release.md) — the lockstep gains the half a bump cannot do. Bumping six strings makes an upgrade *possible*; it does nothing about a target that already holds a copy and never runs an align again, which is how this repository ran `specs.py` at **1.0.0** against a plugin at **4.2.0** with nothing noticing. The doc now owns the rule `skills.py drift` implements: the check runs from the **plugin's** copy and refuses (exit 2) rather than answer from the stale `VERSION` it is being asked about; **both directions** are findings, because resolution is plugin-first while every align leaves a newer copy alone, so an `ahead` copy is never executed and never repaired; and **installed is not wired** — `okf-validate.py` on disk with no `hooks` block is inert, the state this repo sat in, so the wiring question is asked for the one tool that is a hook and never for the two CLIs. Severity follows the cost: `behind`/`unwired` error, `ahead`/`unreadable`/a missing hook warn, and a missing CLI is not a finding at all, because warning about it would fire on every plugin-only repo and a probe whose output is routine noise gets skipped.

**Creation**: [Upgrade the OKF contract to v0.2 or later](/specs/plans/2026-07-28-upgrade-okf-to-v0-2.md) — captures the ask to move the bundle contract off OKF v0.1 to the current release, carrying whatever it adds through into the `/docs:*` command bodies as well as `okf-spec.md` and `okf-validate.py`.

**Creation**: [/specs:develop should not offer to create a follow-up spec](/specs/plans/2026-07-28-stop-develop-offering-follow-up-specs.md) — captures that `develop`'s routed offer duplicates the follow-up row in `/specs:conclude`'s distillation harvest, which already mints follow-ups at the one point where it is known whether the parent spec shipped.

**Creation**: [Resolve a spec from its own worktree before falling back to the current branch](/specs/plans/2026-07-28-resolve-spec-from-worktree.md) — captures the ask that `/specs:execute` prefer the spec file inside the worktree for the slug when one exists, so it builds against the copy carrying the branch's ticked boxes rather than the main checkout's stale view, falling back to the current branch only when no worktree is there.

**Update**: [plans/](/specs/plans/index.md) — two discoveries recorded from the `/align` run, both extending an existing spec rather than opening a new one, because each was already owned. [notice-installed-tool-version-drift](/specs/plans/2026-07-28-notice-installed-tool-version-drift.md) gains the failure mode its Problem missed: the stale copies were not merely three major versions behind, they were **inert** — `settings.json` had no `hooks` block, so nothing invoked them and deleting all three changed no behaviour. A drift check comparing only `--version` would have caught the staleness and still missed the silence, so whatever notices drift must also answer *is this tool wired?*. [retire-skill-vocabulary](/specs/plans/2026-07-27-retire-skill-vocabulary.md) gains the half of its scope that lives in shipped product source rather than in prose: `okf-validate.py`'s own finding message names two retired skills on **every hook firing in every target repo**, and `assets/templates/harness/claude-root.md` — the mold `/docs:harness` writes from — carries five, so both actively re-seed the vocabulary that spec exists to retire. That reframes it from cosmetic to propagating, and is an argument for `/specs:triage` to revisit its level 12.

**Update**: [Scoped hooks](/docs/standards/automation/hooks.md) — the OKF checker is now wired into this repository, and the doc that governs `.claude/settings.json` + `.claude/hooks/**` was updated to say so. The previously installed copies were **v1.0.0** against a plugin at **4.2.0**, and `settings.json` carried **no `hooks` block at all** — the scripts sat on disk with nothing invoking them, which is why deleting them changed no behaviour. Reinstalled `okf-validate.py`, `specs.py` and `hooks-config.json` at 4.2.0 and merged the wiring: a `PostToolUse` hook matched to `Write|Edit` (rung 2) and an unmatched `Stop` hook (rung 3), affordable only because of the shipped `stopScan: "dirty"` gate. The opt-in `PreToolUse` deny gate stays **unwired** (`hardBlock: false`) — the checker proposes, never blocks. Both events were proven by feeding real hook JSON on stdin: silent on a conformant doc, `additionalContext` findings on a deliberately broken one, exit 0 either way. The doc's §"What this repo's own surface does under it" had asserted *"Nothing else on the surface wires a hook"*, which the install falsified — so the `stale-doc` warning against it was accurate and is now closed by fixing the doc rather than the timestamp.

**Update**: [specs/](/specs/plans/index.md) — `/specs:align` aligned the workspace (0 migrated, 0 renamed, 0 shadow copies removed). `doctor` and `validate` were already clean across 36 specs, so the sweep's only work was the one item neither tool checks: `specs/QUENCHING.md` was absent and is now installed at v4.2.0, completing the canonical root. The `plans/` GENERATED zone was already current — the reindex was a no-op — and `okf-validate.py specs/plans --listing-root` passes at 0/0. `specs.py` was **not** reinstalled into `.claude/hooks/`: that copy was deliberately deleted in a prior commit, and a sweep re-creating it would overwrite a human's decision to satisfy a check. Reported, not applied: 28 specs in `plans/` (26 captured, 1 designed, 1 executing) and four live `plan/*` branches, all of which need `/specs:develop`, `/specs:execute` or `/specs:conclude` — none of them a sweep's to drive.

**Update**: [Align](/docs/index.md) — `/docs:align` converged in 1 pass (structure + harness; memory skipped empty, glossary not offered). Structure: [QUENCHING.md](/docs/QUENCHING.md) refreshed v1.0.0 → v4.2.0, which retired three commands it still advertised (`/docs:align-and-update`, `/align-and-update`, `/specs:backlog:add`) and the wrapper model that went with them; `standards/index.md`'s GENERATED zone rebuilt from disk, correcting a stale row that credited [surface-verification.md](/docs/standards/quality/surface-verification.md) with three preconditions after it had grown to five. Harness: the root `CLAUDE.md` went 374 → 106 lines — one MOVE ([versioning-release.md](/docs/standards/ci-cd/versioning-release.md), the first doc in the previously empty `ci-cd/` subject) and six DEDUPE groups replaced by pointers, while the verification commands and the two never-refactor rules stayed as operational content. The product-describing sections (the 25-command table, the three tool descriptions, the OKF contract) were deliberately **not** moved into `docs/`: their canonical home is the product's own source — `plugins/quenching/README.md`, the command frontmatter and `assets/references/**` — and copying them here would create exactly the duplication this repo forbids. 3 advisory validator warnings remain, none of them closable by an align.
