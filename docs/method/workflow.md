...
from scratch for Claude**, **review docs/skills/hooks/agents/CLAUDE.md**, **see
what's missing from the base**, or otherwise describe a mess/gap in the knowledge
surface. It can also be invoked by name (`/claude-quenching:quenching-management`).

The flow is always the same; what changes by scenario is **where the roadmap
weighs** (see the three scenarios below).
...
The 8 steps are constant; the **weighting** shifts with what the repo already
has.
...
3. Confirm installation **item-by-item**. Typically in this order of value:
   ...
   - **canonical `docs/` taxonomy**
     ...
     apply** (a repo without data doesn't get `catalog/`);
...
5. **Human content is not installed:** direction (`docs/vision/`), memory and the
   boundary doctrine the method **only proposes** — you write them.

Result: a coherent `.claude/`/`docs/` with the **recurring cycle** already wired
(Scenario C governs future rounds).
...
1. Ask to **"audit/organize the repo's knowledge"** or **"see what's missing from
   the base"**. For large repos, it can delegate the scan to the read-only
   sub-agent
...
3. Pay attention to two sections of the report:
   - **Deprecables** — skills/agents/hooks the repo already had that a package
     payload now covers. The method **installs the single source** and lists the
     old one; removal is **your decision** (and if the repo's one is better
     adapted, it offers **keeping yours** and discarding the payload).
   - **`docs/` variant names** (`docs/arquitetura/` → `standards/`,
     `docs/adr/` → `decisions/`, single `VISION.md` → `vision/`,
     `docs/catalogo_dados/` → `catalog/`): the method **proposes migration to the
     canonical name** — converge, not preserve — always with OK.
...
Auditing **once** leaves the base fresh **today** — but it **rots** as the code,
models and team change (*context rot*). So the method **doesn't end with the
report**: Step 8 installs the **recurring operational loop** that keeps the
surface alive, with three triggers and the right home for each. In a repo that
already applied the structure, maintenance is **operating those triggers**, not
re-installing:

- **On event, in the PR (drift trigger):** touching `CLAUDE.md`/`docs/` is a doc
  change like any other. A **PR hook** re-runs detection of the touched scope and
  **PROPOSES** (does not block) the gaps.
- **On cadence, at model release (review trigger):** revisit the base when
  ...
  limitation may become **overhead** (a skill/rule the model now does on its own
  ⇒ deprecable).
- **On demand, by command (manual trigger):** the
  ...
  command fires the re-examination (Steps 1–7, or the scope the argument
  requests) and, in large repos, delegates to the auditor sub-agent in clean
  context.

**Automatic freshness between audits** comes from the bundled hooks (see
[Bundled artifacts](../plugin/artifacts.md) for the full table). **Typical
maintenance:** run the re-audit command periodically (or when the PR/Stop hook
signals), treat the report as in Scenario B (install what stayed Absent, fix
Drifted, migrate variants, deprecate the rest), and rely on the hooks for
freshness between rounds.
