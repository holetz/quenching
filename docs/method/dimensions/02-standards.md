...
- Minimal frontmatter (`title` / `updated` / `status` / `audience` / `authority`); one
  standard per file; English kebab-case names.
...
installs only the homes that apply, and regenerates `INDEX.md` from disk. The payload is the
`quenching-docs` and `quenching-standards` skills (the latter fans out one `quenching-writer`
per subject) plus the `docs/` scaffold — see [Bundled artifacts](../../plugin/artifacts.md).
