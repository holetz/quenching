...
- **Portable** — it doesn't impose a foreign structure: naming, language and paths adapt to the repo (where the repo has a *convention*, the repo wins), but the knowledge *structure* converges to the method's standard — and the `docs/` home names follow a canonical taxonomy. This [fixed-vs-adaptive line](method/architecture.md) is explicit throughout the docs.
