...
**Measured, and it is why this repository's bundle root is `docs/` rather than a dotted name.** A
`docs_dir` whose path carries a `.`-prefixed component produces **zero pages**: no `index.html`,
and not even the `404.html` every other build emits. The build prints `Build finished`, reports no
issues, and **exits 0**. Nothing in the output reads as a failure.

The bundle's content is never the cause. Measured on 2026-08-28 against this bundle as it then
stood, at `.knowledge/`:

| `docs_dir` | Its children | Pages built |
| --- | --- | --- |
| `.knowledge/documentation` | all normal | 17 (16 + `404.html`) |
| a copy of that same tree at `knowledge-probe/` | all normal | 107 (106 + `404.html`) |
| a probe root holding one `.hidden/` subfolder | one dotted | the dotted subfolder alone is skipped |
| `.probe-docs` | all normal | **0** |

The last two rows isolate the rule: a dotted entry is pruned wherever it sits, and the `docs_dir`
component is itself part of the path being judged — so a dotted root prunes everything beneath it.

**Symlinks are no escape hatch.** A non-dotted probe root holding
`standards -> ../.knowledge/standards` built only the real files beside the link; the symlinked
tree contributed no page at all.

The consequence for an OKF bundle is structural, not a tuning problem: a bundle root that starts
with a dot can never be the site's source — only a non-dotted subtree of it can. This repository
resolved it at the root, renaming the bundle to `docs/`, after which the same probe rendered all
107 files straight from the root.
