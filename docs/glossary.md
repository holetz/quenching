...
- [**Plugin config**](standards/workflows/plugin-configuration.md) — `.claude/quenching.json`, the
  ...
  `azureColumns`, `subjects`, `tagCatalog`, `workItemTypes` and `gitConventions` — the last of
  which carries the `git` pillar's per-artifact writing directives and outranks the target's own
