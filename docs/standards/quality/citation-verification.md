...
to a `/docs/standards/**` the skeleton does not carry resolves here and in no other
repository.
...
- a **markdown link** to `/docs/**.md` promises a destination, so it is honest only where the
...
2026-08-11). The second is **minting a `/docs/standards/` doc and citing it from shipped
