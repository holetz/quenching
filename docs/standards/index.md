...
| [schema-key-prose-fanout.md](quality/schema-key-prose-fanout.md) | Adding a key to a machine-read schema ages every prose site that spells the record out, and no checker sees it — why the validators are all blind to this by construction, the grep that finds the sites while the change is still cheap, and why it belongs to the task that adds the key rather than to a later sweep |
