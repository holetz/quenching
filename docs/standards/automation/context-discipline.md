...
2. **A reference citation in a command body IS a `§`-address — never a bare path.** Surface-wide
   ...
   **Two command bodies are converted** — `/quenching:specs:execute` and
   `/quenching:specs:develop`; the remaining twenty-three are tracked by the
   `convert-the-remaining-command-bodies-to-section-addresses` spec.
