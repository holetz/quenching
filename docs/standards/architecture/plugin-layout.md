...
(`functional-checks.sh`, `conclude-order-check.sh`). The site-layer payload — `mkdocs/` at the
time, `zensical/` since the generator was replaced — was missing from the inventory line above
entirely.
...
`/docs/` convention, which no configuration names ([bundle-root.md](bundle-root.md)) — and
